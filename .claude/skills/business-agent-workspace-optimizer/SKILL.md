...
- 权限模型：业务 Agent 的项目基线允许 `Bash(*)` 直行，由 sandbox、PreToolUse hook、deny 规则和审计兜底，不走 HITL；不要在普通优化中新增 `ask` 条目或随意放大 allow。除 seed / operator reconcile 明确维护该基线外，反馈优化或单 Agent 调优不得把新高危工具加入 allow；MCP 写/处置工具如需人审，保留在 ask。
