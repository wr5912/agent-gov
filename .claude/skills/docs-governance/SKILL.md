...
4. 对新增或实质修改的权威原则、公开契约和不可逆决策填写最小充分论证卡：实际问题、事实依据、未选替代方案、修订或退出条件。
5. 按动作更新入口和引用：活跃文档进入 `docs/README.md`，归档文档进入 `docs/archive/README.md`，移动或删除前先用 `rg` 检查旧路径引用。
6. 对拆分或合并后的原文档，只保留短入口、替代关系和权威链接，不继续维护重复正文。
...
- “适当增加论证”不是增加篇幅指标；只保留足以理解意图、边界、证据和退出条件的内容，调查流水账留在 issue、PR 或归档。
