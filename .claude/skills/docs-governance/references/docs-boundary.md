...
新增或实质修改权威原则、公开契约、跨层边界或不可逆工程决策时，正文应能回答：

| 检查项 | 必须回答 |
| --- | --- |
| 实际问题 | 解决哪个用户、产品或工程问题？为什么现在需要？ |
| 事实依据 | 哪些代码、机器策略、运行态或已确认约束支持该判断？ |
| 主要替代方案 | 至少一个合理替代方案为何本期不采用？ |
| 边界 | 本文明确做什么、不做什么，谁拥有相关事实？ |
| 验收 | 如何证明已实现，而不是只有文档声明？ |
| 退出条件 | 哪些事实变化会触发重审、升级或删除该规则？ |

最小充分不等于固定篇幅。简单 runbook 可以用前置条件、动作、成功证据和失败处理完成论证；
宪法原则则必须说明事实基础与修订条件。调查命令、聊天过程和临时时间线不应为了“显得有论证”
而进入长期正文。
...
- 只有结论口号、没有事实依据与退出条件的“原则”不得进入宪法；反过来，也不得用复杂远期设计填充当前阶段的最小契约。
