...
测试深度的硬性要求以 `.codex/guidance/verify.md`（Claude 侧 `.claude/rules/verify.md`）为准：生命周期状态变更加非法转移用例、并发资源加竞态/部分失败用例、外部输入加异常/无效/超范围用例、AI 自动化输出契约加后端所有权字段污染用例。本技能只负责「增删改判断」，深度清单不在此复制。
