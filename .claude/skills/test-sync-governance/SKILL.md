...
| 改动类型 | 推荐验证命令 | 升级条件 |
| --- | --- | --- |
| `.codex` / `.claude` skill、README、docs 容器治理 | `git diff --check`、`scripts/check_docs_governance.py`、`scripts/check_codex_governance.py --mode fail`、相关 skill/governance 单测 | 用户要求完整验证、发版、或改动影响运行时代码 |
...
| 产品主流程、Agent job、formatter、store、API/UI 状态 | `make main-flow-test` + 相关 pytest | 提交/CI/发版时跑 `make test` |
| v2.7 UI 设计一致性、抽屉/modal 语义、Playground 动作边界 | `pnpm --dir frontend run verify:design-parity`，必要时追加真实容器 `RUNTIME_UI_BASE` 验收 | 改动真实前端组件时同时跑 `pnpm --dir frontend build` |
