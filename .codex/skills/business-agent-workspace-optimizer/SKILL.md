...
| `main` 样板 | 样板 workspace | `${RUNTIME_ROOT}/main-workspace` 或 `docker/runtime-template/main-workspace` | 谨慎，只能作为样板 | 用户明确要求改 main | 模板扫描或运行态重渲染 |
| governor | 治理 Agent workspace | `${RUNTIME_ROOT}/governor-workspace` 或 `docker/runtime-template/governor-workspace` | 仅用户明确要求 | 说明这是治理 Agent，不是业务 Agent | 同步治理 Agent 相关验证 |
