...
| --- | --- | --- | --- | --- | --- |
| 单个业务 Agent | 业务 Agent workspace | `${RUNTIME_ROOT}/data/business-agents/<agent_id>` | 是，且必须精确到单个业务 Agent | `agent_id`、registry `workspace_dir` 或用户给出的绝对路径 | JSON/Markdown/权限/脱敏扫描 |
| `main` 样板 | 样板 workspace | `${RUNTIME_ROOT}/main-workspace` 或 `docker/runtime-volume-seeds/main-workspace` | 谨慎，只能作为样板 | 用户明确要求改 main | 模板扫描或运行态重渲染 |
...
| runtime 父目录 | runtime data 容器 | `${RUNTIME_ROOT}/data` 或 `${RUNTIME_ROOT}/data/business-agents` | 否 | 只是父目录，不是 workspace | no-op 并重新定位 |

- 离线解析 agent_id → workspace（不要直接读 `runtime.sqlite3`，不得把 `${RUNTIME_ROOT}/data` 或 `${RUNTIME_ROOT}/data/business-agents` 父目录作为修改目标）：
  - 运行态业务 Agent：约定路径 `${RUNTIME_ROOT}/data/business-agents/<agent_id>` 指向单个业务 Agent workspace（容器 `/data/business-agents/<agent_id>`，本机调试 `/tmp/local-debug-volume-agent-gov/data/business-agents/<agent_id>`）；或经运行中的 `GET /api/agent-registry` 查 `workspace_dir`。
  - 模板：仓库内 `docker/runtime-volume-seeds/<workspace>`，当前为 `main-workspace`、`governor-workspace`（五个治理 workspace 已合并为单一 governor）；业务 Agent 暂无模板目录，新增模板按同一规则。
...
- `${RUNTIME_ROOT}/data/business-agents/<agent_id>/`（必须精确到单个业务 Agent；可编辑配置：`CLAUDE.md`、`.claude/`、`.mcp.json` 等）
- `${RUNTIME_ROOT}/main-workspace/`（样板，谨慎）
- 仓库 `docker/runtime-volume-seeds/<workspace>/`
