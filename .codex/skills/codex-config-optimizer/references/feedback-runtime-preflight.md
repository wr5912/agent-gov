...
用于 `claude-agent-runtime` 中反馈优化批次、runtime-template/env、Agent job、DSPy formatter、回归资产和用户可见闭环问题。目标是在改代码前用 5 行固定住证据链和验证点，避免把常驻规则继续写长。
...
5. 验证绑定：目标 pytest/UI verification nodeid 是什么；是否需要同步 tests/coverage_policy.json 并运行 make main-flow-test。
```
...
- 如果第 5 行找不到现有覆盖，先补目标测试或 coverage policy 场景，再确认实现。
