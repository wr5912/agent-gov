...
- 是否影响产品主流程；如果影响，是否已经找出对应主流程 manifest、coverage policy 或等价测试清单？
...
- 如果任务涉及反馈闭环、Agent job、DSPy formatter、Pydantic OutputModel、store projection、OpenAPI 或前端生成类型，是否已经产出契约整改预检五项？
...
- 涉及反馈闭环、Agent job、DSPy formatter、Pydantic OutputModel、store projection、OpenAPI 或前端生成类型时，产出契约整改预检五项：产品症状证据链、字段所有权矩阵、执行动作矩阵、治理风险预检和验证矩阵。
...
- 如影响用户可见主流程，说明 manifest / coverage policy 如何同步，UI 空态、成功态和失败态错误详情如何验证。

完成后：

- 运行相关测试、构建、类型检查或人工验证。
- 涉及 Codex 配置治理或 skill 优化时，运行 `codex-config-optimizer` 的只读审计脚本，或说明无法运行原因。
- 涉及反馈闭环、Agent job、DSPy formatter、Pydantic OutputModel、store projection、OpenAPI 或前端生成类型时，在主要 schema/store/formatter 改动完成后先运行中段治理 checkpoint：fail 模式治理硬门和 touched-files ruff，再继续补功能或测试。
...
- 产品主流程变更：必须覆盖 UI/API/job/store/formatter 的关键映射；项目如配置主流程 manifest 或 coverage policy，必须同步场景绑定。
