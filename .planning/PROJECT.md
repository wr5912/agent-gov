...
- ✓ per-Agent lane 的 3 个 plans / 12 个 tasks 已完成；固定 verifier、candidate snapshot v5、最小权限 worker/sandbox、durable receipt、lifecycle lock、stale/signal cleanup 和发布门均已落地。文档冻结前功能收口轮已通过，收尾提交继续由文档冻结后的 exact-tree durable final gate 约束 — Phase 7。
...
- Phase 7 已完成（3/3 plans、12/12 tasks）：API 只创建 durable run，固定 profile 只映射批准 verifier；caller env 在首次 bootstrap 前清理，绝对工具链与 `agentgov.container-acceptance-candidate.v5` 候选快照共同固定执行来源。回执按 `reserved -> prepared -> single terminal` 持久化，并受 lifecycle lock、stale recovery 与 signal commit point 约束。
- Runtime bootstrap 已进入候选镜像，Compose 不再允许宿主 bind 或 `RUNTIME_BOOTSTRAP_HOST_DIR`；删除 authority 由 0059、fd-relative Workspace fingerprint、Git metadata/temp authority、Settings request context、defensive boundary 与 TIA 精确绑定共同收口。各公共入口保留独立 fresh receipt，不能合并伪装成一张全覆盖回执。
- 2026-08-13 最终文档冻结候选的 collect-only 基数为：quality manifest 2225 leaves、`main-full` 2220 leaves、host no-Docker acceptance 402 leaves、main-flow backend 694 leaves（640 个精确 selector）、UI 11 bindings、OpenAPI 134 operations；此前七个公共入口已在共同候选 tree/env 摘要上分别形成历史功能轮 terminal receipt。收尾提交只允许在本文冻结后 exact-tree `make test` 与七入口 durable final gate 全部成功时产生，最终回执不回填本文以避免改变已验候选。
...
- 当前发布版仍为 3.0.3；v3.1 处于 executing，完成 Phase 7 不触发 `VERSION` 或 tag 变更。
...
| 业务 Agent 测试不进入根静态 collection | 测试正文归属 Workspace Git，平台只治理 runner、隔离、lane 和 receipt | ✓ Phase 6 的 58 leaf 仍只归属该 Workspace；Phase 7 exact-commit 隔离执行与发布门已验收 |
| 容器验收 profile 只映射固定 verifier | caller 不能选择私有 target、command、env、toolchain 或 receipt 身份 | ✓ 固定 verifier、caller-env scrub、candidate snapshot v5、durable receipt 与全部公共入口 fresh evidence 已通过 |
| Runtime bootstrap 使用镜像内候选来源 | 宿主 bind 会绕过 frozen image/source authority | ✓ Dockerfile 内置 bootstrap，Compose/验收拒绝临时 bind 与 `RUNTIME_BOOTSTRAP_HOST_DIR`，同候选容器门通过 |
