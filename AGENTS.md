...
- 网络安全运营 Agent、安全缺陷整改、不可信输入测试或安全评审：`defensive-security-boundary`
...
- AgentGov 是智能体治理平台；内置网络安全业务 Agent、示例和测试仅服务于用户拥有并授权环境
  的防御性监测、研判、加固和响应处置。
...
- 修改反馈主流程、Agent job、formatter、store、API response 或用户可见 tab 时，运行
  `make main-flow-test`，并同步 `tests/quality_policy.json` 的场景绑定。
- `tests/quality_policy.json` 是覆盖率、资产分类、owner、lane、主流程、TIA、并行晋级、
...
- 生命周期加非法转移测试；并发资源加重复/竞争/部分失败测试；外部输入加异常/无效/超范围测试；
  Agent 输出契约加冲突的 backend-owned 字段污染与后端权威保护测试。
- 提交、发版或用户要求完整验证时运行 `make test`；专项覆盖率调整运行 `make coverage`。
...
`warn`/dry-run 只用于 Analyze，不能作为 Verify 通过标准。提交和部署前的质量结论以本地
`make test` 及按风险选择的真实容器验收为准。
...
- 不把本仓库脚本名、自动化流水线、路径或产品不变量复制到团队通用 skill/rules 模板。
