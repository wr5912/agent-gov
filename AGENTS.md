...
- 能从仓库确认的事实，先通过只读方式确认。
- 执行架构饱和检查：确认待修改文件行数、类/函数/路由规模、复制粘贴风险、状态/Schema 表示是否分散。
- 显式说明关键假设、歧义和技术权衡。
...
5. 验证方式和成功标准。
6. 架构阈值命中情况；如命中，说明先拆分、抽象或统一的方案。

### Execute

- 仅在用户确认后写文件。
- 只改完成目标所需的文件和逻辑。
- 优先复用现有模式、工具和约定；但命中架构阈值时，不得继续向饱和文件追加新职责。
...
- 回答架构验证项：是否新增超限文件、重复块、状态字面量分散、Schema 双轨、跨事务中间态或文档漂移。
...
- 不为一次性逻辑创建抽象。
- 不添加未被要求的灵活性、配置项或扩展点。
- 如果实现命中架构阈值或明显臃肿，先收缩、拆分或抽象后再继续。

### 精准改动

- 每一行修改都应能追溯到用户目标。
- 匹配项目现有风格，即使个人偏好不同。
- “匹配现有风格”不得作为继续复制大段代码、扩大神模块或分散状态机的理由。
...
- `status` / `phase` / `stage` 等状态字段的合法转移散落在多个分支中。
- 同一实体同时存在 Pydantic 模型与手写 dict 双轨表示。
- 跨表、跨文件或 DB + 文件系统更新被拆成多个不可恢复的事务步骤。
- 修改路由、settings、Docker env、公开 schema 或默认值但不同步 README / docs 对应章节。

命中阈值不意味着必须大重构；默认做法是先抽出当前改动必需的最小边界，再完成原目标。

## 测试纵深

- 涉及状态字段时，至少补充 1 个非法状态转移或非法状态输入的负向测试。
