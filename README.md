...
`security-operations-expert` 是当前唯一内置、默认且受保护的网络安全业务 Agent。它的三个属性由平台分别派生，不使用来源字段合并表达。该 Workspace 可以按字节导出、以任意新 Agent ID 导入并继续开发；导出包是可修改的 Workspace 起点，不是平台模板。精确的 `soc_api__create` / `soc_api__manual` 由导入 Workspace 的 Claude 原生 `permissions.ask` 逐次确认，工具参数不可由决策请求改写，高风险请求不提供 run 级放行；API 统一使用 `API_KEY`。初始化源只处理整体缺失的内置 Workspace，不回灌现有实例；现有实例升级应走“导出候选 → 跨 ID 导入测试 → 导出验证包 → CAS 覆盖目标 → 回归/发布”流程。
...
- Workspace 测试与平台运行：`GET /api/agent-registry/{agent_id}/test-suite`、手工运行 `POST /api/agent-test-runs`、待发布变更运行 `POST /api/agent-change-sets/{change_set_id}/test-runs`、`GET /api/agent-test-runs`、`GET /api/agent-test-runs/{test_run_id}`、`POST /api/agent-test-runs/{test_run_id}/cancel`，以及 `agentgov_testkit` 使用的 `/api/agent-test-sessions` 和 `/api/agent-test-sessions/{test_session_id}/messages`。测试内容只来自精确 Git 提交中的 `workspace/tests/`；平台固定执行 `python -m pytest -q -p agentgov_testkit.pytest_plugin tests`，客户端不能提交命令、状态、业务归属或报告。
- 版本治理：`GET /api/agent-repository` 及 current/snapshot/discard-changes，`POST/GET /api/agent-change-sets` 及 events/diff/file-diff，发布使用 `POST /api/agent-change-sets/{change_set_id}/publish`，`GET /api/agent-releases` 及 restore/rollback。普通发布要求同一业务 Agent、当前待发布 `commit_sha` 上存在 `passed` 的平台测试运行；旧提交通过不能放行新提交。强制发布必须提供非空原因并持久化原阻塞项和警告，provenance 不完整不可绕过。

3.0.0 不保留旧评测链兼容层。SQLite migration 0040 归档并删除更早的全局用例/场景包链；migration 0048 归档后删除数据库测试集、旧评估运行及待发布变更上的历史评测字段，并建立平台测试运行表；migration 0049 收口四阶段测试产物命名；migration 0051 原样归档旧的自然语言测试设计并替换为 pytest 代码候选契约。旧名称只允许出现在迁移、归档和旧入口不存在的负向测试中。
...
示例：`docker/runtime-bootstrap/business-agents/security-operations-expert/workspace/.claude/agents/response-playbook-builder.md`

```markdown
---
name: response-playbook-builder
description: 根据已确认的安全运营判断构建处置步骤和验证清单。
tools:
  - Read
  - Grep
  - Glob
...
示例：`docker/runtime-bootstrap/business-agents/security-operations-expert/workspace/.claude/skills/security-operations-analysis/SKILL.md`

```markdown
---
name: security-operations-analysis
description: 对安全运营事件进行证据优先的分析、归因与处置建议。
allowed-tools:
  - Read
  - Grep
  - Glob
  - mcp__sec-ops-data__*
context: fork
agent: response-playbook-builder
---
...
每个业务 Agent 的权限真相位于其运行态 `workspace/.claude/settings.json`。内置 `security-operations-expert` 的初始化配置示例如下：
...
- 该内置业务 Agent 的 Bash 走 `ask`；`allow_for_run` 只绑定 `business_agent_id + run_id + low-risk category`，高风险或未分类请求不能整轮放行。
...
上述权限和 hook 仅是内置业务 Agent 的初始化配置，不是所有新 Agent 的平台默认值。容器启动不迁移或修复已有 live Workspace；已有 Agent 需要调整时，应直接修改其 Workspace 并通过 per-Agent Git/版本治理留痕。`requires_web_hitl` 仅是从 Workspace `permissions.ask` 派生的只读观测值，权限真相仍只在 project settings。
