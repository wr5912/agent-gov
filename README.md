...
只新增或修改 `docker/runtime-volume-seeds/` 下的预制业务 Agent 配置时，在已使用当前 Compose 配置 recreate 过容器后，不需要重建镜像；执行 `make up` 由 API 启动协调器独占完成缺失 workspace 播种、Git 版本源初始化和运行 receipt 写入。修改 Dockerfile、Python 代码、bootstrap 渲染逻辑或依赖时仍需 `make build` 并 recreate。
...
可复用模板保存在 `docker/runtime-volume-seeds/`。API 是共享运行卷的启动协调者；worker 不执行初始化，也没有独立的 `agent-gov-runtime-init` 容器。API/worker 正常运行时各自持有共享租约，允许闭环优化、job 状态和 Git 版本等合法并发写入；bootstrap、受管策略迁移和 receipt 更新必须持有独占租约。启动完成后，`${DATA_DIR}/.agent-gov/runtime-coordination/receipt.json` 绑定代码/seed/env 摘要、运行模式和卷身份。worker 只有在 API 健康且 receipt 有效时才启动，API 失联后停止领取新 job。

已有业务 Agent workspace 是其 Git 版本真相源，bootstrap 只在整个 workspace 缺失时写入出生配置，不会逐文件回灌 seed。受管策略升级只自动迁移 Git 工作树干净且没有未终结 change set 的 Agent，并生成 `managed_policy_migration` 快照；否则 API 启动失败，要求先处理脏文件或 change set。Compose 中 `RUNTIME_VOLUME_SEEDS_HOST_DIR` 默认是 `./runtime-volume-seeds`（相对 `docker/docker-compose.yml`），以只读 bind mount 覆盖镜像内 `/app/docker/runtime-volume-seeds`；部署目录不包含该模板时 Compose 会失败。真实运行态文件默认不提交到本仓库。
...
`runtime-bootstrap` / `local-debug-bootstrap` 通过同一协调器准备并验证运行卷；`runtime-validate` / `local-debug-validate` 只读校验 receipt 与受管策略。运行态配置回滚依赖每个业务 Agent 自己的 Git version/release，不依赖散落备份文件。
...
本机 PyCharm 调试如果需要访问本机 Docker 暴露的 HTTP MCP 服务，在 `docker/.env.local-debug` 中设置 `MCP_SERVER_URL=http://localhost:58001/mcp`，然后执行 `make local-debug-bootstrap`；协调器会在独占阶段迁移干净的受管配置并写入 receipt。main profile 和 feedback profiles 都只使用各自 workspace 的官方 `.mcp.json`。宿主机存在代理变量时，同时在 `CLAUDE_ENV_JSON` 中设置 `NO_PROXY` 和 `no_proxy`，避免本机地址请求被代理转发。
