...
只新增或修改 `docker/runtime-volume-seeds/` 下的预制业务 Agent 配置时，在已使用当前 Compose 配置 recreate 过容器后，不需要重建镜像；执行 `make up` 会先停止 API/worker、由新 API 独占完成运行卷准备，再在 API 健康后启动 worker。修改 Dockerfile、Python 代码、bootstrap 渲染逻辑或依赖时仍需 `make build` 并 recreate。
