...
- 通过四套 Runtime Profile 隔离主智能体、归因分析智能体、优化方案生成智能体和执行优化智能体：`/main-workspace`、`/attribution-analyzer-workspace`、`/proposal-generator-workspace`、`/execution-optimizer-workspace` 与独立 `claude-roots/*`。
...
为减少 bind mount 权限问题，Compose 中的 API 容器默认以 root 运行，启动时会对 `docker/volume/data/`、四个 workspace 和 `docker/volume/claude-roots/*` 对应的容器挂载目录执行 `chmod -R a+rwX`，方便直接写入。生产环境如果需要收紧权限，可以再切换到非 root 用户并配套处理宿主机目录 owner/ACL。
