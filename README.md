...
│   ├── runtime-volume-seeds/        # 可复用运行卷模板（运行目录由 bootstrap/entrypoint 从此渲染）
...
可复用模板保存在 `docker/runtime-volume-seeds/`，实际运行目录由 `make runtime-bootstrap`、`make local-debug-bootstrap` 或容器 entrypoint 从模板补齐。真实运行态文件默认不提交到 git；其中 `runtime.sqlite3`、临时 job、Agent 版本仓库、候选 worktree、发布归档和 Langfuse 运行数据都属于本地运行态。
