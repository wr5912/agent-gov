...
运行态目录不在仓库内：容器部署默认根为宿主机 `${HOME}/volume-agent-gov`，本机调试默认 `/tmp/local-debug-volume-agent-gov`，由 API 启动协调器或 `make runtime-bootstrap` / `make local-debug-bootstrap` 从 `docker/runtime-volume-seeds/` 原样播种缺失 workspace。Compose 默认把当前 checkout 的 `docker/runtime-volume-seeds/` 只读挂到 API 容器内，Docker 镜像内仍保留一份 seed 作为兜底。其下含 `governor-workspace/`、`claude-roots/governor`、`data/`（`business-agents/<id>/{workspace,claude-root,version}`——含预制 main-agent；`runtime.sqlite3`、证据/任务临时文件、候选 worktree、发布归档）和可选 `langfuse/`，均默认不提交。`docker/volume/` 不是当前布局，仅在显式沿用旧目录时作为迁移来源或兼容路径。
...
只新增或修改 `docker/runtime-volume-seeds/` 下的预制业务 Agent 配置时，在已使用当前 Compose 配置 recreate 过容器后，不需要重建镜像；执行 `make up` 由 API 启动协调器独占完成缺失 workspace 原样播种、Git 版本源初始化和运行 receipt 写入。修改 Dockerfile、Python 代码、bootstrap 复制/校验逻辑或依赖时仍需 `make build` 并 recreate。
...
- 业务 Agent 与 workspace 资产：`GET/POST /api/agent-registry`、`POST /api/agent-registry/{agent_id}/workspace/export`、`POST /api/agent-registry/{agent_id}/workspace/import`、`POST /api/agent-registry/{agent_id}/workspace/restore`。创建时可用互斥的 `template_id` 或 `source_seed_id`；声明 seed 跨 ID 原样复制，不重写身份散文或权限。live workspace 包可包含真实 endpoint 和私有运行配置，导出包应按敏感资产保管；导入、恢复和导出 snapshot 都绑定 per-Agent Git commit，下一 turn 生效。repo seed/builtin 是另一条仓库边界：实例化原样不代表带真实秘密的 live workspace 可无条件原样提交进 Git。
...
- `/data/runtime.sqlite3` 是反馈信号、SOC 事件、处置单、证据包 manifest 和文件内容、改进事项、四阶段内容子资源、typed TestDataset、EvalRun 和 API session 的权威存储；升级前遗留的 `agent_jobs` 行作为只读历史保留。
...
可复用模板保存在 `docker/runtime-volume-seeds/`。API 是共享运行卷的启动协调者；没有独立的 `agent-gov-runtime-init` 容器。bootstrap、缺失 workspace 播种和 receipt 更新必须持有独占租约。启动完成后，`${DATA_DIR}/.agent-gov/runtime-coordination/receipt.json` 绑定代码/seed/env 摘要、运行模式和卷身份。

已有业务 Agent workspace 是其 Git 版本真相源，bootstrap 只在整个 workspace 缺失时原样复制出生配置，不会逐文件回灌 seed，也不会因 receipt 或运行模式变化自动改写 settings、MCP 或 hook。Compose 中 `RUNTIME_VOLUME_SEEDS_HOST_DIR` 默认是 `./runtime-volume-seeds`（相对 `docker/docker-compose.yml`），以只读 bind mount 覆盖镜像内 `/app/docker/runtime-volume-seeds`；部署目录不包含该模板时 Compose 会失败。真实运行态文件默认不提交到本仓库。
...
live workspace 通过产品 workspace 导出 API 按精确 Git commit 原样导出；AgentGov 不再提供从
整个 runtime volume 生成或恢复 repo template 的并行命令。需要纳入声明 seed/builtin 时，
先在仓库外保留逐字节候选，再人工选择要提交的 `workspace/` 内容并运行
`make runtime-volume-seeds-scan`。真实密钥、MCP 私有 header、数据库凭据和本机私有路径会阻断
提交；非秘密 endpoint、内网地址和较宽权限只提示复核，不会被静默改写。`runtime-volume-seeds-clean`
...
- 默认允许读文件、搜索文件、调用已配置的只读 MCP，以及把报告类产物写入运行态输出目录。
- 通用业务 Agent 的 Bash 走 `ask`；`allow_for_run` 只绑定 `business_agent_id + run_id + low-risk category`，高风险或未分类请求不能整轮放行。
...
上述权限和 hook 是新 seed/template 的出生基线。容器启动不迁移或修复已有 live workspace；已有 Agent 需要调整时，应直接修改其 workspace 并通过 per-Agent Git/版本治理留痕。`requires_web_hitl` 仅是从 workspace `permissions.ask` 派生的只读观测值，权限真相仍只在 project settings。
