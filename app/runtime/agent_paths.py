...
``workspace`` 是 Git 管理的 Harness 事实源；``version_base`` 保存候选 worktree 与
发布归档。Session、Message 和运行态 Workspace 均由独立 AgentScope Runtime 持有，
因此 AgentGov 不再为每个 Agent 保存第二份 Runtime home。
"""
