...
from .errors import BusinessRuleViolation, ConfigurationError
from .external_governance_models import ExternalGovernanceItemRecord, ExternalGovernanceNotificationRecord
...
class ExternalGovernanceService:
    ...
    def notify_item(
        ...
        *,
        ...
        with self.Session.begin() as db:
            ...
            row = db.get(ExternalGovernanceItemModel, external_item_id)
            if row:
                record = self._item_record_from_row(row).with_notification(
                    updated_at=utc_now(),
                    notification=notification,
                )
                self._apply_item_record(row, record)
    ...
    def item_to_dict(self, row: ExternalGovernanceItemModel) -> dict[str, Any]:
        item = self._item_record_from_row(row).to_payload()
        with self.Session() as db:
    ...
    def _item_record_from_row(self, row: ExternalGovernanceItemModel) -> ExternalGovernanceItemRecord:
