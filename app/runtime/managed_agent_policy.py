...
def validate_security_agent_yaml(text: str, *, path: str) -> tuple[PolicyViolation, ...]:
    ...
    violations: list[PolicyViolation] = []
    if not isinstance(agent, dict) or agent.get("profile") != SECURITY_OPERATIONS_EXPERT_AGENT_ID or agent.get("requires_web_hitl") is not True:
        violations.append(PolicyViolation(SECURITY_OPERATIONS_EXPERT_AGENT_ID, path, "security_agent_identity", "profile/requires_web_hitl"))
