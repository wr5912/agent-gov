...
def regression_test_design_prompt(*, prompt_context: JsonObject | None = None) -> str:
    return _structured_prompt(
        ...
        (
            ...
            "必须且只能调用一次 result = agent.run(原始输入)，第一条结果断言必须逐字写成 `assert not result.errors`；"
            "errors 是 tuple，禁止写 `result.errors == []`。禁止 mock、patch、test double、动态代码执行、替换 agent.run，"
            "也禁止重新赋值 result 或修改 result.text、result.raw、result.errors 及其嵌套内容。"
