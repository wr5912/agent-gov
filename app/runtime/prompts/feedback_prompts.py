...
def attribution_prompt(input_path: str) -> str:
    return (
        ...
        "通常判定为 problem_type=tool_unavailable 或 runtime_error，optimization_object_type=mcp_config，"
        "actionability=workspace_config_change。"
        "workspace_placeholder_summary.json 中其他占位符按来源归因："
        ".claude/settings.json 若影响权限、sandbox 或网络域名，通常归 optimization_object_type=runtime_code、actionability=runtime_fix；"
        "mcp_servers/**/sample*.json 若作为 MCP 工具返回数据污染回答，通常归 optimization_object_type=external_mcp_service、problem_type=tool_data_quality；"
        ...
        "通常判定为 optimization_object_type=runtime_code，actionability=runtime_fix。"
