...
本文说明如何把 `volume/workspace/mcp_servers/` 中的示例 MCP 服务替换为你本地开发的 MCP 服务，并让 Claude Code 智能体使用它们。
...
```text
volume/workspace/mcp_servers/my_soc_mcp/
├── server.py
├── requirements.txt
└── README.md
```
...
```bash
-v /your/local/mcp/my_soc_mcp:/workspace/mcp_servers/my_soc_mcp
```
...
```json
{
  "mcpServers": {
    "soc-data": {
      ...
      "args": ["${CLAUDE_WORKSPACE:-/workspace}/mcp_servers/my_soc_mcp/server.py"],
      "env": {
...
```bash
cd /workspace/mcp_servers/my_soc_mcp
npm install
npm run build
```
...
```json
{
  "mcpServers": {
    "soc-data": {
      "type": "stdio",
      "command": "node",
      "args": ["${CLAUDE_WORKSPACE:-/workspace}/mcp_servers/my_soc_mcp/dist/server.js"],
      "env": {
        "SOC_API_URL": "${SOC_API_URL:-http://host.docker.internal:8080}",
        "SOC_API_TOKEN": "${SOC_API_TOKEN:-}"
      }
...
```text
volume/workspace/.claude/settings.json
```
