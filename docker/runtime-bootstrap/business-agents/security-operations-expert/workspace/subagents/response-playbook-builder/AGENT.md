...
2. 其他只读会话先调用 `mcp__sec-ops__soc_api__get_resp_playbooks_recommend`，再通过 `mcp__sec-ops__resource_read(uri)` 读取剧本详情 template 实例并逐个核对推荐候选。复用前必须读取最新详情，并确认全部 ACTION `properties.plugin_id`（或 `actionSummary[].pluginId`）均匹配当前 action-defs 的 `actionKey`、已启用且不是 `simulated=true`；任一动作失效即排除该剧本。推荐候选全部排除后直接生成临时剧本，不使用 Bash 或 AgentScope Runtime 内部 tool-results 文件穷举超大目录。仅当推荐接口明确不可用且剧本列表能被 MCP 完整返回时，才读取 `openapi://soc_api/resp/playbooks` 兜底。
