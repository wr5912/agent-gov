...
> 整改注记（2026-05-30）：当前 `.codex/` 不再存放项目债务账本，`.codex/size-budget.yaml` 不存在；治理硬门以 `scripts/check_codex_governance.py --mode fail` 对比 git base 执行行数、函数、类、路由与状态机缺表检查，且已接入 `.codex/hooks.json` Stop hook、`make test` 与 GitHub Actions。本文涉及 size-budget、空 hook 或 warn 模式的段落保留为发现时的治理反思，不代表当前仓库结构。
...
| §5.D 文档同源（OpenAPI→TS 生成） | ⚠️ OpenAPI→TS 生成链路已落地；CI 漂移阻断仍未完全接入 | 仍需防止生成物与代码漂移 |
| §5.E 软门 → 硬门 | ✅ `make test`、Stop hook 与 CI 均运行 `--mode fail` | 新增可判定债务会被阻断 |

**当前对账结论**：R1 方案方向正确；早期问题是只完成了"写规则"，没完成"让规则有牙齿"。当前已补上 fail 模式、Stop hook 与 CI 阻断，剩余重点是继续把复杂度、重复、schema 漂移等更深结构问题机器化，避免再次退回到人工自觉。
