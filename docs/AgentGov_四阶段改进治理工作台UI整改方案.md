...
- 普通发布入口；
- 受保护的强制发布入口、必填原因和持久化警告。

旧提交通过、空测试目录、失败、错误、取消或服务重启导致的 `interrupted` 都不能满足普通发布条件。变更返工后产生新待发布提交时，必须重新运行该提交的测试。
...
旧提交通过、空测试目录、设计已确认但文件未生成、失败、错误、取消或中断都不能放行。强制发布必须走受保护入口、填写非空原因，并把原阻塞项、操作人、原因和警告持久化；provenance 不完整不可强制绕过。
