...
#### 2）管理（可编辑）
适用于会改变对象关系或资产内容的面板，打开对应管理抽屉。适用：来源反馈、测试用例 / 测试数据集。
...
lifecycle_state
quality_tags[]
provenance.regression_assessment_id
provenance.source_feedback_ids[]
provenance.baseline_agent_version_id
provenance.candidate_agent_version_id
cases[].case_id
cases[].position
cases[].prompt
cases[].expected_behavior
cases[].checkpoints[]
created_at
updated_at
```
...
```text
draft -> active -> evaluating -> active
  |         |            |
  |         +----------> deprecated -> active
  |         |            |
  +---------+------------+----------> archived
...
- `draft`：从已确认事项证据链采用，尚未进入执行；
- `active`：可作为回归运行输入；
- `evaluating`：正在作为回归数据集执行，完成后返回 active；
- `deprecated`：不再默认用于新运行，但可经审计恢复 active；
- `archived`：保留审计，不再参与新回归选择，且不可恢复。

`evaluating` 是 EvalRun 拥有的运行态，不能由 UI 或通用生命周期 API 手工伪造。每次人工生命周期变更携带 `expected_revision`；非法转移、并发覆盖、运行中抢占和跨 Agent 误归属必须被后端拒绝。
...
- 重新生成回归评估：基于当前反馈、归因、优化方案和候选变更生成用例候选；
- 采用测试集：从已确认的事项证据链派生不可伪造 backend-owned 字段和有序用例；
- 执行回归：绑定当前数据集 revision，固化完整用例快照后运行候选版本；
- 人工复核：仅在当前 EvalRun 的自然语言语义无法机械判定时，逐 case 选择通过或拒绝，并填写复核人与理由；
- 生命周期治理：按 revision CAS 将数据集激活、评估、废弃或归档。

这些动作放在“测试资产与计划”面板或其管理抽屉中，不放入决策卡。决策卡只保留“执行回归测试”和必要的返回动作。

采用完成后，用例计数与详情只读取 `TestDataset.cases` 的固化快照，不再回退到可能继续变化的
`RegressionAssessment.cases`。生命周期操作必须提交目标状态、原因和当前 revision；修订记录可追溯，
加载失败必须显示错误与重试入口。回归执行状态和门禁预览从 change set 绑定的持久化 EvalRun 恢复，
刷新页面不能退回“等待执行”；终态 change set 的 worktree cleanup pending 也必须提供可达的重试动作。
待复核结果必须显示独立面板和 `regression_review_required` 状态；提交只调用绑定当前 ChangeSet/EvalRun 的专用 review API。决策未覆盖全部 case、复核人或理由为空时按钮禁用。复核不修改原始 item/check 证据，也不能借强制发布按钮替代；全接受后才开放普通发布，任一拒绝继续阻断。
...
因此，`TestDataset` 必须进入统一资产视图并与以下对象建立 provenance，但保持 typed store/API，不能降格序列化为通用 `AssetRecord.body`：
...
旧全局用例、独立回归资产和场景包主路径不再定义改进治理工作台，也不再暴露公开 API。

新的关系是：

```text
测试发布阶段
  -> 确认 RegressionAssessment 候选
  -> 采用 / 管理 typed TestDataset
  -> 执行 EvalRun 不可变快照
  -> 通过发布门禁
  -> 在 AssetRegistry 中查看数据集 provenance
  -> 发布
```

也就是说，TestDataset 本身就是有归属、修订、生命周期和 provenance 的长期 typed 资产，不再复制成第二套回归资产实体。

### 13.7 验收断言

后续代码整改必须验证：

- 当前测试集不是匿名数组，必须有 `dataset_id`；
- 测试数据集必须有 `agent_id` 和 `improvement_id` 归属；
- 测试数据集必须记录来源反馈、用例列表、基线版本和候选版本；
- 回归运行必须引用具体 `dataset_id` 与启动时固化的 revision；
- 资产 Registry 必须直接投影 typed TestDataset，不创建平行 ID 或 body 内 schema；
...
7. **AI / 人类双友好同源**：人类主界面保持低噪声，AI / Playwright / 测试数据自动化工具通过 ContextPackage 获取完整结构化上下文；两者必须来自同一事实源。
