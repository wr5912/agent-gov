...
补充权威规则：`docs/AgentGov_四阶段改进治理工作台UI整改方案.md` 与四张效果图是改进治理工作台
目标 UI、流程、入口、决策卡和处理记录的最高依据。其他文档中的旧七段链路、旧反馈工作台菜单
或旧发布入口与其冲突时，按四阶段方案解释或废除；当前实现、OpenAPI、测试执行协议和里程碑状态
仍由各自权威来源维护。
...
| `反馈信息` | 已退出主导航的旧反馈工作台用户对象 | `Feedback` | 只在历史 UI、迁移说明或负向测试中保留；当前主流程用“反馈”。 |
...
| `optimization batch` / `优化批次` | 已删除旧链路中合并反馈并生成方案的容器 | 由 `ImprovementItem` 聚合和阶段推进承接 | 只在历史迁移、归档材料和旧入口不存在的负向测试中保留。 |
| `proposal` / `optimization proposal` | 已删除旧链路中的方案对象和 job 输出 | `OptimizationPlan` | 只在历史迁移、归档材料和旧入口不存在的负向测试中保留；内部通用 JSON key 不构成领域对象。 |
...
| `反馈信息 / 优化批次 / 回归资产 / 版本管理` | 已退出主导航的旧反馈工作台四菜单 | 四阶段改进治理工作台中的来源、方案、测试资产和发布条件能力 | 只作历史 UI 证据，不作为当前主导航或验收结构。 |
...
| 反馈优化 workspace | 已退出主导航的旧反馈闭环工作台 | 四阶段改进治理改进事项闭环的历史能力来源 | 只作兼容诊断或历史证据；不得重新成为当前产品主入口。 |
