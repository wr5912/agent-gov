...
3. 下一大版本 v2.7 规划，例如 `AgentGov_v2.7_四阶段改进治理工作台UI整改方案.md`；旧 ASCII 草图和对应设计一致性报告已进入归档，只作为历史设计证据。
...
| 当前实现基线（pre-v2.7） | `反馈闭环当前实现基线.md`、版本治理、多业务 Agent 基座相关方案 | 保留真实实现名称，开头声明与 v2.7 的关系 | 当前仍解释运行态，不因 v2.7 存在而移动 |
| 下一大版本 v2.7 规划 | `AgentGov_v2.7_四阶段改进治理工作台UI整改方案.md` | 使用 v2.7 统一术语；改进治理工作台以四阶段方案为最高依据 | 属于规划权威，不能替代当前实现基线 |
