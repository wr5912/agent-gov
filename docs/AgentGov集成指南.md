...
- **新增预制业务 Agent 不再要求重建镜像**：在已使用当前 Compose 配置 recreate 过容器后，新增 `docker/runtime-volume-seeds/data/business-agents/<agent_id>/workspace/` 后执行 `make up`；API 启动协调器会从只读 seed 播种缺失 workspace、初始化该 Agent 的 Git 版本源、写入运行 receipt，并在就绪后启动 API 服务。修改 Dockerfile、Python 代码、bootstrap 渲染逻辑或依赖仍需重建镜像。
