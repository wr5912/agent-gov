...
- **不要改 `docker/runtime-template/`**：那是 AgentGov 给**新建** Agent 渲染初始 workspace 的**种子模板**（随镜像分发、共享），不是某个业务 Agent 的运行配置；改它只影响后续新建/重渲染，定制不到已存在的 Agent。定制某个具体 Agent 一律改其运行卷 workspace。
- 边界与注意：
