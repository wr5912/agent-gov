...
- 边界与注意：
  ...
  - 权限：Bash 不走 HITL，业务 Agent 基线用 allow + sandbox/hook/deny/审计兜底；MCP 写入/处置工具如需在线人审，放入 `ask` 并通过 `/api/chat/stream` + `ENABLE_CLAUDE_WEB_HITL=true` 承载。配置文件里不写 api_key / token / 凭据。
