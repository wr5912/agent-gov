...
- 最短路径：
  ...
  - OpenAI 兼容：`POST /v1/chat/completions`（便于复用现成 OpenAI 客户端）。OpenAI 标准请求无 `agent_id` 字段，故该入口**固定以 main agent 运行、无法指定业务 Agent**；要指定具体 Agent 用上面的 `/api/chat` 或 `/api/chat/stream`。
