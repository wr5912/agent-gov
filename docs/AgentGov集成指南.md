...
- **部署形态**：Docker 容器对外提供 HTTP API，供 Web UI、业务系统、Agent 平台控制面调用。**Base URL 由部署方提供**：外部 / 同主机默认 `http://<host>:48080`（宿主暴露端口 `HOST_PORT`；当前实现分支临时使用 `4XXXX` 端口段，避免与 master 默认 `5XXXX` 服务并行冲突；本机调试常见 `http://localhost:48080`），同 Docker 网络内的服务用 `http://claude-agent-api:8080`；容器内 app 端口是 `8080`，生产可能在反向代理 / TLS 之后。
...
`ENABLE_CLAUDE_WEB_HITL=true` 时，Web 人工确认只随 `/api/chat/stream` 生效；非流式 `/api/chat` 使用 bypass 权限模式，不会承载确认卡。集成方如果要在自己的聊天界面展示“人工确认卡”，必须把 `/api/chat/stream` 当成带暂停点的 SSE 状态机，而不是普通文本流。
...
- `tool_permission`：工具授权卡。动作是 `allow_once`、`allow_for_run`、`deny`。`allow_for_run` 只在同一 `business_agent_id + run_id` 内跳过后续工具确认，不跨 run、不写入永久权限配置。
...
- **配置位置 = 该业务 Agent 的运行卷 workspace**：`${HOST_RUNTIME_VOLUME_ROOT}/data/business-agents/<agent_id>/workspace/`（容器内 `/data/business-agents/<agent_id>/workspace`；`<agent_id>` 即 `/api/agent-registry` 创建时的 id，其 `workspace_dir` 也可由 `GET /api/agent-registry` 查到）。可定制：
  ...
  - 该 `workspace/` 是这个 Agent 的**活配置层**（git 就地版本化的版本源）；与它**并列**在 `<agent_id>/` 下的还有 `claude-root/`（Claude SDK 运行态）与 `version/`（per-agent 版本治理工件：worktrees/releases）——去嵌套后运行态天然不进版本源。优化闭环跑的是候选 worktree、发布归档落 `version/releases/` 或 `data/agent-governance/`，都不在 `workspace/` 内。
...
- **不要改 `docker/runtime-volume-seeds/`**：那是 AgentGov 渲染运行卷初始态的**种子**（镜像卷结构：`governor-workspace/`、预制 `data/business-agents/<id>/workspace/`、`templates/business-agent/` 创建模板；随镜像分发、共享），不是某个业务 Agent 的运行配置；改它只影响后续新建/重渲染，定制不到已存在的 Agent。定制某个具体 Agent 一律改其运行卷 `workspace/`。
- **种子 = 出生配置，运行卷 = 活优化态（强制不可覆盖）**：bootstrap 对 `data/business-agents/*` 只做**存在性对账**——种子里新增的预置 Agent（卷里缺失）会被渲染补全并自动登记，但**已存在 Agent 的 workspace 配置绝不被覆盖**，即便最激进的 `--overwrite`/`--repair-managed-config` 也强制跳过该子树，保护反馈优化闭环 publish 进卷的优化成果与 per-agent git 历史不丢失。配套：种子预置的业务 Agent 是**声明式基线**，UI 与 `DELETE /api/agent-registry/{id}` **禁删**（要移除请改 `docker/runtime-volume-seeds/` 源）；用户经创建接口注册的非种子 Agent 可删除（tombstone 逻辑删除，重启不复活）。
- 边界与注意：
