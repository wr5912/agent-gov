...
- 创建一致性：`POST /api/agent-registry` 只有在模板完整预渲染、workspace 缺失文件安全落盘并完成注册表 finalize 后才返回 `201`；内部 `provisioning` reservation 不会被 list/get/chat 看见。文件或 finalize 失败会补偿注册表并只清理本次创建的文件，预存 workspace 不覆盖、不整目录删除；模板中的 symlink、路径穿越、`__pycache__`/字节码缓存，以及 workspace 目标路径中的任一 symlink 均 fail closed。reservation 使用 15 分钟 heartbeat 租约，新 API 进程不会恢复仍在落盘的创建；过期任务由启动检查和每分钟 reconciliation 恢复旧 tombstone，或转为不可见 tombstone 后允许安全重试。
...
2. 渲染标准 Responses 事件；control mode 同时处理 `agentgov.session`、`agentgov.tool_step` 等事件。收到 SSE comment heartbeat 时只刷新连接存活时间。
...
- `ask_user_question`：Claude 主动向用户澄清。动作只能是 `answer_question`，结构化选项或自然语言回答统一放入顶层 `answer` 对象。
...
- **新增预制业务 Agent 不再要求重建镜像**：在已使用当前 Compose 配置 recreate 过容器后，新增 `docker/runtime-volume-seeds/data/business-agents/<agent_id>/workspace/` 只需重启 API/worker，entrypoint 会从只读挂载的 seed 重新 bootstrap，缺失的业务 Agent 会补到 `${HOST_RUNTIME_VOLUME_ROOT}/data/business-agents/<agent_id>/workspace/` 并在应用启动时登记到 `/api/agent-registry`。修改 Dockerfile、Python 代码、bootstrap 渲染逻辑或依赖仍需重建镜像。
...
- 边界与注意：
  ...
  - 权限：Bash 不走 HITL，业务 Agent 基线用 allow + sandbox/hook/deny/审计兜底；MCP 写入/处置工具如需在线人审，放入 `ask` 并通过 `/v1/responses` control mode（`stream=true`）+ `ENABLE_CLAUDE_WEB_HITL=true` 承载。配置文件里不写 api_key / token / 凭据。
