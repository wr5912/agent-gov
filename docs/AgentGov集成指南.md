...
- **部署形态**：Docker 容器对外提供 HTTP API，供 Web UI、业务系统、Agent 平台控制面调用。**Base URL 由部署方提供**：外部 / 同主机默认 `http://<host>:58080`（宿主暴露端口 `HOST_PORT`，约定 50000 + `API_PORT`，默认 58080；本机调试常见 `http://localhost:58080`），同 Docker 网络内的服务用 `http://claude-agent-api:8080`；容器内 app 端口是 `8080`，生产可能在反向代理 / TLS 之后。
...
- 最短路径：
  ...
  - 流式：`POST /api/chat/stream`（Server-Sent Events），`agent_id` 同样**必填有效**（与 `/api/chat` 口径一致，不静默跑 main）。
  - OpenAI 兼容：`POST /v1/chat/completions`（便于复用现成 OpenAI 客户端）。OpenAI 标准请求无 `agent_id` 字段，故该入口运行**运营者配置的出口 Agent**（经 `GET/PUT/DELETE /api/settings/openai-compat-agent` 配置或前端设置 UI 选择；**未配置时默认走 main**，与显式选 `main-agent` 是不同状态）；要按请求指定 Agent 用上面的 `/api/chat` 或 `/api/chat/stream`。
...
- **配置位置 = 该业务 Agent 的运行卷 workspace**：`${HOST_RUNTIME_VOLUME_ROOT}/data/business-agents/<agent_id>/`（容器内 `/data/business-agents/<agent_id>`；`<agent_id>` 即 `/api/agent-registry` 创建时的 id，其 `workspace_dir` 也可由 `GET /api/agent-registry` 查到）。可定制：
  - `CLAUDE.md`（角色/边界/SOP）、`.claude/skills/<skill>/SKILL.md`、`.claude/agents/*.md`、`.claude/rules/*`、`.mcp.json`（工具接入，并同步 `.claude/settings.json` 权限）。
  - 该目录就是这个 Agent 的**活 workspace**（注册时由底座渲染生成，**不是**优化闭环产出的版本快照）；它分三层：**配置层**（上面这些可编辑文件，定制只改这一层）、`version/`（per-agent git 版本库）、`claude-root/`（Claude SDK 运行态）。优化闭环跑的是 `data/agent-governance/worktrees/` 里的**候选副本**、发布归档落 `data/agent-governance/releases/`，都不在该 workspace 内。
  - 位置不对称（设计如此）：内置常驻 Agent `main` / `governor` 各有专属顶层目录 `${HOST_RUNTIME_VOLUME_ROOT}/main-workspace/`、`/governor-workspace/`；动态注册的业务 Agent **没有**顶层 `*-workspace`，其 workspace 即 `data/business-agents/<agent_id>/`。无论哪种，定制 / 优化的目标都是该 Agent `workspace_dir` 下**配置层**的可编辑文件。
- **不要改 `docker/runtime-volume-seeds/`**：那是 AgentGov 给**新建** Agent 渲染初始 workspace 的**种子模板**（随镜像分发、共享），不是某个业务 Agent 的运行配置；改它只影响后续新建/重渲染，定制不到已存在的 Agent。定制某个具体 Agent 一律改其运行卷 workspace。
- 边界与注意：
  - **不要碰** 该 workspace 下的 `version/`（该 Agent 的 per-agent git 版本库）与 `claude-root/`（运行态 Claude 状态）；改它们会破坏版本治理 / 运行态。
  - 该 workspace 同时是 Agent 的 git 版本源——直接改是工作树未提交改动；要固化成版本走版本治理（change set / release，见 4.6）。
  - 权限：后端为非交互运行时，`ask` 权限无法呈现会被无声阻断——只用 allow/deny + 最小权限，人审在 `CLAUDE.md` 对话级表达；配置文件里不写 api_key / token / 凭据。
...
若上层系统本身用 Claude Code / Codex 开发，可安装 AgentGov 提供的可分发集成 skill（见仓库 `integrations/agentgov-claude-skill/`），让集成方的开发 Agent 直接掌握上面的旅程与边界。该 skill 派生自本指南，本指南为单一真相源。
