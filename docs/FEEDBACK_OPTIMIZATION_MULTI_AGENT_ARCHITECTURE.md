...
```text
当前已实现四个智能体 = 四套 Runtime Profile；开发人员在优化方案中点击具体任务的“执行”即表示确认，随后 execution-optimizer 生成执行方案，后端完成校验并受控应用。
```

核心规则如下：

```text
1. 四个 Agent 可以运行在同一个容器中。
2. 四个 Agent 可以由同一个 FastAPI 后端进程调度。
3. 四个 Agent 不共享 workspace。
4. 四个 Agent 不共享 claude-root。
5. 四个 Agent 必须使用不同的 cwd。
6. 四个 Agent 必须使用不同的 HOME。
7. 四个 Agent 必须使用不同的 CLAUDE_CONFIG_DIR。
8. 四个 Agent 可以共享 /data，但必须按 job_id、case_id、agent role 分区。
...
3. 直接输出 `feedback-optimization-plan-output/v1`，其中 `tasks` 是可执行任务，`blocked_items` 是不能执行或不能派发的事项。
...
5. 任务记录 applied_agent_version_id、pre_execution_agent_version_id、execution_job 和 applied_diff。
6. 开发人员手动触发回归评估。
...
```text
1. 四个 Agent 共用 /root/.claude。
2. 四个 Agent 共用同一个 CLAUDE_CONFIG_DIR。
3. 四个 Agent 共用同一个 workspace。
...
[ ] attribution-analyzer-workspace、proposal-generator-workspace 和 execution-optimizer-workspace 已创建。
[ ] claude-roots/main、attribution-analyzer、proposal-generator、execution-optimizer 已创建。
[ ] 四个 profile 的 cwd 不同。
[ ] 四个 profile 的 HOME 不同。
[ ] 四个 profile 的 CLAUDE_CONFIG_DIR 不同。
