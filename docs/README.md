...
这些文档解释当前代码、API、数据库、测试和用户可见运行态。文档中出现的 `main-agent`、`反馈信息`、`优化批次`、`proposal` 等名称属于当前实现事实；四阶段改进治理用户主流程术语以 [AgentGov术语与版本边界](./AgentGov术语与版本边界.md) 为准。它们不得作为改进治理工作台未来主流程依据；与四阶段方案冲突时，以 [AgentGov 四阶段改进治理工作台 UI 整改方案](./AgentGov_四阶段改进治理工作台UI整改方案.md) 和四张效果图为准。
