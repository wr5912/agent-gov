# 业务 Agent Workspace 包导入与热加载产品工程方案（复杂安全旧稿）

> 文档状态：历史设计稿。本文保存复杂导入状态机、内容扫描和生产级安全边界的旧方案，
> 不再定义当前产品契约。当前方案见
...
- `metadata_updated` 只恢复 previous name，不制造空 Git commit。除校验当前 HEAD 与 receipt 记录一致外，
  ...
  仅用 HEAD 给 name 做条件更新是错的：`metadata_updated` 按定义 tree 不变、HEAD 恒定，等于用常量做乐观锁，
  对并发 name 写入零保护；name 是注册表列且无版本令牌，只能按旧值做条件更新保护。
...
| `rollback_failed` | `rollback_reserved`，且仅限重新验证预期版本和无未对账副作用时 |
| `rolled_back` | 无 |
...
首版只允许已经获得 AgentGov API Key 的可信内部开发者导入。该假设降低的是发布者身份和不可信代码执行隔离
...
- 不可信内容/依赖扫描、隔离的验证执行器和网络/CPU/内存/PID/文件配额。
...
- validation、active turn、change set、maintenance、条件更新冲突次数。
...
- rollback 预期版本校验、重复 rollback、后续 HEAD 已变化、metadata-only rollback。
- 连续两次 `metadata_updated` 后回滚第一次，必须以 `409 IMPORT_METADATA_CHANGED` 拒绝：HEAD 在两次操作间
  恒定不变，只有按旧 name 值做条件更新才能拦住它，该用例是区分“有效条件更新”与“常量锁”的唯一手段。
