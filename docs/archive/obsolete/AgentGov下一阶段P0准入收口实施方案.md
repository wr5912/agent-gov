...
> 归档说明（2026-09-09）：本阶段形成于旧 Claude Runtime epoch，已经完成其历史评审使命；
> 其中 `sdk_session_id`、Claude hooks 和既有卷口径不属于当前 AgentScope 原子切换后的实现契约。
> 当前 Runtime、fresh epoch 与容器验收以根 `README.md` 和 `docs/AgentGov集成指南.md` 为准。

> 文档状态：评审稿。
>
> 工程阶段说明：P0 是下一轮研发准入阶段，不是四阶段改进治理工作台的用户阶段。
>
> 历史索引：[AgentGov 下一阶段实施方案索引](../../AgentGov下一阶段实施方案索引.md)。
...
- P1、P2B 分别绑定 AGV 用例、质量 owner 和验收入口；AgentScope Runtime 作为已落地基线
  单独通过 Gateway、容器与 trace 契约验收。
...
验证 AgentScope 原生工具事件与结构化结果语义，退出码只作为进程诊断证据。

另保留两项不计入上述 14 个失败的分类：

- MCP 工具权限转交继续 `KEEP`，保持 AgentScope Runtime 权限流程，后端不按 Agent ID 接管；
...
1. 当前部署只启用 AgentScope Runtime，客户端请求不能选择或覆盖 Runtime、模型、凭据和
   Harness。`BusinessAgentVersion -> RuntimeBinding` 由后端持有，会话在创建时固定版本；
...
该裁决以根 `README.md` 的 AgentScope Runtime 公共契约为准；后续若调整恢复语义，必须同步修改
状态机、OpenAPI、集成指南和非法转移测试，避免再次形成两个活跃真相源。
...
| AgentScope Runtime 基线 | AGV-002、014、040、050 | `runtime-platform` + `integrations` | Gateway、AgentEvent SSE、暂停/恢复、OTel trace、真实容器 |
