...
> 归档说明（2026-09-09）：本文记录旧 Claude SDK/Responses/Speech Summary 路线，仅用于历史
> 审计；生产执行与观测已由 AgentScope Runtime 和 OTel 语义 trace 原子取代。
>
> 状态：Accepted / 实施依据；决策日期：2026-07-28；目标版本：3.0.3；公开契约真相源：OpenAPI
> 历史上位架构依据：[OpenAI 兼容接口能否替代原生 Chat 端点评估](./OpenAI兼容接口能否替代原生Chat端点评估.md)
