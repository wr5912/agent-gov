...
> 归档说明（2026-09-09）：本文的 Responses-first 与 Claude SDK 过渡实现已被 AgentScope Runtime
> 原子切换取代，仅用于历史审计，不再代表当前或下一阶段生产契约。
>
...
>
> 历史实现曾以已删除的 Responses/Claude 路由为准。当前外部接入权威说明见 [AgentGov 集成指南](../../AgentGov集成指南.md)；早期 HITL [实现方案](../design/Claude原生业务Agent人类确认机制整改实现方案.md)与[对抗审查计划](../design/Claude原生业务Agent人类确认机制对抗审查整改计划.md)仅保留历史审计价值。
