...
> 归档说明（2026-09-09）：本文记录旧 Claude/LiteLLM sidecar 路线，仅用于历史审计；生产模型
> 与 MCP 凭据现只注入 AgentScope Runtime，不再部署协议 sidecar。
>
> 状态：已落地的工程基线与后续验收方案
> 日期：2026-07-14
...
10. **内部 sidecar 不要求用户配置 `LITELLM_MASTER_KEY`**。`MODEL_PROVIDER_API_KEY` 属于 sidecar 到上游模型服务的凭据；`LITELLM_MASTER_KEY` 属于客户端到 LiteLLM Proxy 管理面的管理员凭据，二者不得复用。当前 sidecar 不发布宿主机端口并关闭 Admin UI/Swagger/Redoc；若未来变成共享网关，必须重新设计独立管理员密钥与应用 Virtual Key。
11. **API 启动期后台预热完整 provider 能力门**；首个模型请求与预热共享 single-flight，成功结果进进程内缓存。预热失败只降级 readiness，不阻断控制面 liveness。
...
- LiteLLM 官方把 master key 定义为 Proxy Admin key，用于管理 Virtual Keys；官方同时提供 `DISABLE_ADMIN_UI=True`、`NO_DOCS=True` 与 `NO_REDOC=True` 关闭管理展示面。AgentGov 当前只使用内部推理协议转换能力，不启用 Proxy Admin / Virtual Key 管理能力。
  参考：<https://docs.litellm.ai/docs/proxy/virtual_keys>、<https://docs.litellm.ai/docs/proxy/ui>、<https://docs.litellm.ai/docs/proxy/configs>
...
- Compose 已包含 LiteLLM sidecar；API 启动后在后台执行完整的 single-flight provider capability warmup，具体模型请求复用同一路由、同一成功缓存，并在传输故障时 fail-fast。
...
| LiteLLM sidecar | container | 使用同一个 `MODEL_PROVIDER_API_URL` 访问 vLLM 模型服务；禁用 Admin UI 功能并关闭 Swagger/Redoc | 无持久业务数据 | `MODEL_PROVIDER_API_KEY` 仅访问上游；无用户配置的 proxy master key；不发布宿主机端口 | sidecar `/health`、UI discovery 禁用态、文档路由 404、version/capability probe |
...
- `MODEL_PROVIDER_API_KEY` 是模型服务凭据，只允许传给上游 adapter；它不是 LiteLLM Proxy Admin key，不得映射、复制或派生为 `LITELLM_MASTER_KEY`。
- 当前 sidecar 是 Compose 内部实现细节：不发布端口，设置 `DISABLE_ADMIN_UI=True`、`NO_DOCS=True`、`NO_REDOC=True`，且不启用 Proxy Admin / Virtual Key 管理。用户不配置 `LITELLM_MASTER_KEY`。
- 若未来 sidecar 对外暴露或成为多应用共享网关，必须退出当前内部信任边界：生成独立 Proxy Admin key、给应用签发最小权限 Virtual Key、设计轮换和网络访问控制；不得复用上游模型 key。
...
### 5.2 启动预热与 single-flight

- 实际问题：完整能力门包含 version、sidecar、models、chat、tool loop、schema 和 Claude Messages 兼容探测，若只在第一条 Playground 请求执行，会把数秒固定成本错误归到用户回复时延。
- 事实依据：能力门成功结果对同一进程、同一 route 可复用；并发重复探测只增加模型与 sidecar 压力，不产生新的运行事实。
- 决策：lifespan 后台调用完整 warmup；warmup 与请求都通过同一锁和 `_agent_runtime_ready` 缓存。健康端点只读快照，仍不等待外部服务。
- 未选方案：不把探测改成无条件启动阻塞，因为 provider 不可用时 API/UI 控制面仍必须存活；也不在 warmup 与首请求之间维护第二套任务缓存。
- 修订条件：若后续支持进程内动态切换 provider route，必须在配置切换事务中显式失效 route 与 capability 缓存，再重新 single-flight 预热。
...
| 条件 | 路由 | 结果 |
| --- | --- | --- |
| `MODEL_PROVIDER_API_URL` 以 `/v1` 结尾 | 不发网络请求 | 配置错误，返回 `VLLM_BASE_URL_INVALID`（retryable=false），要求改为服务基地址 |
...
- 配置以 `/v1` 结尾：`vllm_base_url_must_not_end_in_v1`，不发网络请求并返回 `VLLM_BASE_URL_INVALID`
...
配置以 `/v1` 结尾时，事件改为 `VLLM_BASE_URL_INVALID`，`action=reject_invalid_vllm_base_url`；该错误不允许伪装成可重试的网络故障。
...
当前部署还固定以下信任边界：sidecar 只加入 Compose 网络且不发布宿主机端口；Admin UI、Swagger 和 Redoc 均关闭；不启用 LiteLLM Proxy Admin / Virtual Key 管理，因此不要求 `LITELLM_MASTER_KEY`。LiteLLM 1.88.1 在无 master key 时会固定输出一条 CRITICAL 提示，entrypoint 只精确过滤该已知文案，其他 CRITICAL/ERROR 不降级、不吞掉。
...
- sidecar Compose 环境不得接收 `LITELLM_MASTER_KEY` 或完整 API env；必须关闭 Admin UI/Swagger/Redoc且不发布宿主机端口。
- sidecar 配置必须把 `MODEL_PROVIDER_API_KEY` 只写入上游 `litellm_params.api_key`，不得写入 `general_settings.master_key`；已知无 master key 提示可过滤，其他真实 CRITICAL 必须保留。
- `MODEL_PROVIDER_API_URL` 以 `/v1` 结尾：不访问网络，返回 `VLLM_BASE_URL_INVALID` 且 endpoint 脱敏。
- 启动 warmup 与并发首请求：完整 capability probe 恰好执行一次；warm cache-hit gate 的专项 P95 门为 `<=100ms`。
...
- LiteLLM sidecar `/.well-known/litellm-ui-config` 返回 `admin_ui_disabled=true`（1.88.1 的 `/ui` 保留静态禁用提示壳），`/docs`、`/redoc` 返回 404，容器日志不再出现要求用户配置 `LITELLM_MASTER_KEY` 的误导性 CRITICAL。
...
| vLLM URL 是否允许以 `/v1` 结尾 | 不允许；`/version` 与 `/v1/*` 都从服务基地址派生，错误配置在网络前返回 `VLLM_BASE_URL_INVALID` |
...
| `MODEL_PROVIDER_API_KEY` 与 `LITELLM_MASTER_KEY` 是否复用 | 不复用；前者访问上游模型服务，后者属于 Proxy Admin 管理面。当前内部 sidecar 不启用管理面，也不要求用户配置后者 |
| 未来 sidecar 对外暴露怎么办 | 退出当前内部信任边界，单独设计 Proxy Admin key、应用 Virtual Key、网络控制和轮换；不能把上游模型 key 当代理管理员 key |
