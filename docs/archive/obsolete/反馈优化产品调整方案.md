...
> 归档说明（2026-06-23）：本文已从活跃阅读路径移出，仅保留历史设计和审计价值。文中的 `proposal-generator`、`execution-optimizer` 等旧 profile/workspace 表述属于合并前设计；当前运行态治理执行者已收敛为单一 `governor`，当前基线见 `docs/反馈闭环当前实现基线.md`。

> 用途：指导反馈优化功能从“反馈处置单主导”调整为“反馈信息主导”的正式产品开发。
> 状态：v1 已实现并持续校准
> 日期：2026-05-28
> 文档层级：当前实现基线（pre-v2.7）。
> 术语口径：本文保留 `反馈信息`、`优化批次`、`proposal-generator`、`execution-optimizer`、`agent_jobs` 等当前实现名称，用于描述真实运行态；v2.7 用户主流程术语以 [AgentGov术语与版本边界](./AgentGov术语与版本边界.md) 为准。
> 归档边界：本文仍解释当前反馈优化主流程和迁移来源，暂不移动；待 v2.7 改进事项闭环功能等价覆盖后再评估归档。
> v2.7 覆盖规则：本文只说明已落地的 pre-v2.7 反馈优化工作台；`反馈信息 / 优化批次 / 回归资产 / 版本管理` 旧菜单不再定义未来用户主流程。后续改进治理工作台的 UI、阶段、效果图验收和代码整改原则以 [AgentGov v2.7 四阶段改进治理工作台 UI 整改方案](./AgentGov_v2.7_四阶段改进治理工作台UI整改方案.md) 及四张效果图为准。
