...
> 术语口径：本文保留 `反馈信息`、`优化批次`、`proposal-generator`、`execution-optimizer`、`agent_jobs` 等历史实现名称，用于描述迁移来源；当前用户主流程术语以 [AgentGov术语与版本边界](../../AgentGov术语与版本边界.md) 为准。
> 归档边界：本文仍解释当前反馈优化主流程和迁移来源，暂不移动；待 v2.7 改进事项闭环功能等价覆盖后再评估归档。
> v2.7 覆盖规则：本文只说明已落地的 pre-v2.7 反馈优化工作台；`反馈信息 / 优化批次 / 回归资产 / 版本管理` 旧菜单不再定义未来用户主流程。后续改进治理工作台的 UI、阶段、效果图验收和代码整改原则以 [AgentGov 四阶段改进治理工作台 UI 整改方案](../../AgentGov_四阶段改进治理工作台UI整改方案.md) 及四张效果图为准。
