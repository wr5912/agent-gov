...
> 归档说明（2026-06-23）：本文已从活跃阅读路径移出，仅保留历史设计和审计价值。文中的五个治理 Agent 及其 workspace/profile 表述属于合并前设计；当前运行态治理执行者已收敛为单一 `governor`，当前基线见 `docs/反馈闭环当前实现基线.md`。

> 用途：指导 AgentGov / 网络安全运营 AI 助手的反馈优化闭环编码实现。
> 状态：当前实现校准稿
> 版本：v1.1
> 日期：2026-05-26
> 文档层级：当前实现基线（pre-v2.7）。
> 术语口径：本文保留 `main-agent`、`feedback signal`、`feedback case`、`optimization batch`、`proposal-generator` 等历史架构事实；当前用户主流程术语以 [AgentGov术语与版本边界](../../AgentGov术语与版本边界.md) 为准。
> 归档边界：本文仍解释当前闭环架构、运行隔离和迁移风险，暂不移动；若未来被 v2.7 架构文档完全替代，应先迁移引用和测试锚点。
> v2.7 覆盖规则：本文保留历史闭环架构和运行隔离事实；若本文按旧批次、旧治理 Agent 职责名、旧版本管理或旧回归资产入口描述用户主流程，全部让位于 [AgentGov 四阶段改进治理工作台 UI 整改方案](../../AgentGov_四阶段改进治理工作台UI整改方案.md) 及四张效果图。

本版本按当前代码实现校准。文档中的“当前实现”描述的是本仓库已落地能力；标注为“预留”或“待实现”的内容不得作为已完成验收项。

> Governor 合并（Issue #3）已落地：下文按职责区分的归因 / 优化方案 / 执行 / 用例治理 / 回归影响分析五个治理 Agent，运行态已合并为单一治理 Agent `governor`（单一 `governor-workspace` 与 `claude-roots/governor`）。后端按 job_type 复用同一执行者身份，各 job 的 prompt 与 typed-output 契约不变；闭环链路（反馈→归因→优化→评估→版本）不变。下文五治理 Agent 的 workspace / claude-root 分表描述合并前的隔离设计，作为职责说明保留，详见 `docs/engineering/Governor合并设计.md`。
...
```text
当前运行态实现 main + governor 两套 Runtime Profile（归因 / 方案 / 执行 / 用例治理 / 回归影响分析五类治理职责由单一 `governor` 按 job_type 复用，见 §0 合并说明）；下文按六个 Agent 分述的隔离规则描述的是合并前设计，作为职责与隔离语义说明保留。需要模型执行的反馈闭环任务统一先创建 `agent_jobs`，由 worker 异步执行。开发人员在优化方案中点击具体任务的“执行”即表示确认，随后 execution-optimizer 生成执行方案，后端完成校验并受控应用。
```
...
> Governor 合并（Issue #3）已落地：运行态 profile / workspace / claude-root 的权威命名为下表 `main-agent` + `governor` 两套。合并前按职责分述的 `attribution-analyzer` / `proposal-generator` / `execution-optimizer` / `eval-case-governor` / `regression-impact-analyzer` 及其 `*-workspace`、`claude-roots/*` 为隔离设计命名，已并入 `governor` / `governor-workspace` / `claude-roots/governor`；下文沿用这些职责名仅作闭环链路与职责说明，不再作为运行态 profile/目录命名（详见 §0 与 `docs/engineering/Governor合并设计.md`）。

| 名称 | 含义 | 是否可变 |
| --- | --- | --- |
| `main-agent` | 主智能体profile | 固定使用 |
| `governor` | 单一治理 Agent profile，按 job_type 承担归因 / 方案生成 / 执行优化 / 用例治理 / 回归影响分析五类职责 | 固定使用 |
| `main-workspace` | 主智能体workspace | 固定使用 |
| `governor-workspace` | 治理 Agent workspace | 固定使用 |
| `claude-roots/main` | 主智能体claude root | 固定使用 |
| `claude-roots/governor` | 治理 Agent claude root | 固定使用 |
| `feedback case` | 一组反馈和关联证据形成的处置单 | 固定使用 |
| `evidence package` | 针对某个 case 固化后的证据包 | 固定使用 |
| `attribution job` | 归因分析任务 | 固定使用 |
| `optimization plan job` | 优化方案生成任务 | 固定使用 |
| `eval case governance job` | 长期回归用例治理任务 | 固定使用 |
| `regression impact analysis job` | 回归失败影响分析任务 | 固定使用 |
| `optimization proposal` | 历史兼容和审计投影中的内部优化方案产物 | 兼容使用，不作为用户审批对象 |
| `optimization task` | 用户确认执行后的执行任务 | 固定使用 |
...
1. 自行修改自己的核心配置文件，除非通过开发人员确认后的受控优化流程。
...
4. 直接确认或执行 optimization plan task。
5. 把证据不足的问题包装成确定性结论。
...
3. 输出足够完整的优化方案内容，由 DSPy Signature 规范化为 `FeedbackOptimizationPlanOutput`；其中 `tasks` 是可执行任务，`blocked_items` 是不能执行或不能派发的事项。
...
${HOME}/volume-agent-gov/
  ...
  data/
    ...
    agent-governance/
      worktrees/<change_set_id>/... # 候选变更 worktree
      releases/<release_id>.tar.gz  # 发布归档
...
7. Agent job 输入、输出和错误都以 SQLite 为权威存储；内部智能体通过后端构造的 prompt context 获取任务上下文，不读取 job 临时目录。
```
...
services:
  ...
  claude-agent-worker:
    ...
    volumes:
      ...
      - ${HOST_RUNTIME_VOLUME_ROOT}/eval-case-governor-workspace:/eval-case-governor-workspace
      - ${HOST_RUNTIME_VOLUME_ROOT}/regression-impact-analyzer-workspace:/regression-impact-analyzer-workspace
      - ${HOST_RUNTIME_VOLUME_ROOT}/claude-roots:/claude-roots
      - ${HOST_RUNTIME_VOLUME_ROOT}/data:/data
```
...
POST /api/feedback-cases/{id}/optimization-plan
...
4. job input、raw output、validated output、error 以 SQLite 为权威存储；后端在运行前将 SQLite、证据包和 Langfuse trace 汇总为 prompt context，不再临时物化 job 输入文件。
...
归因分析智能体不应直接四处查询散落数据；后端应从 evidence package 和 Langfuse trace 构造 prompt context 后交给智能体。
...
SQLite 是权威存储；Agent job 运行时不再物化 evidence 文件，后端直接读取 `evidence_files.content_json` 并构造 prompt context。
...
```json
{
  ...
  "evidence_package_id": "evp-20260521-000001",
  "main_agent_version_id": "main-v1.2.0",
  "evidence_package": {"manifest": "..."},
  "evidence_files": {
    "feedback.json": ["..."],
    "tool_calls.json": ["..."],
    "trace_summary.json": ["..."],
    "langfuse_trace_details.json": ["..."]
  },
  "langfuse_trace_details": ["..."],
...
```json
{
  ...
  "evidence_package_id": "evp-20260521-000001",
  "attribution_job_id": "fba-20260521-000001",
  "attribution_output": {"status": "completed", "root_causes": ["..."]},
  "main_agent_version_id": "<current-main-commit-sha>",
  "main_agent_repository_path": "/workspace",
  "agent_change_set_worktrees_path": "/data/agent-governance/worktrees",
  "agent_release_archives_path": "/data/agent-governance/releases",
...
```text
1. `FeedbackOptimizationPlanOutput` 必须经过 DSPy formatter 和 Pydantic Schema 最终校验。
...
4. 离线模式不等于无模型模式；部署必须提供本地或内网模型能力，系统不再生成“未配置模型提供商”的 offline/raw 临时输出。
...
`/workspace` 的 Git 提交和发布 tag 是主 Agent 行为包版本源；`/data/agent-governance/worktrees/` 只保存候选变更 worktree，`/data/agent-governance/releases/` 保存发布归档。`/data/feedback-signals/`、`/data/soc-events/`、`/data/pending-correlations/`、`/data/feedback-cases/`、`/data/evidence-packages/`、`/data/optimization-proposals/`、`/data/optimization-tasks/` 等目录仅为兼容路径或历史说明，不是权威事实源。旧版 `/data/feedback/*.jsonl` 和旧版规则生成的 proposal 不参与新版闭环。
...
`optimization-plan` 可携带单次补充指令；该指令只影响本次新建 optimization plan job，不保存为反馈单长期偏好：
...
### 18.10 Feedback Eval and Regression Assets
...
POST /api/feedback-optimization-batches/{batch_id}/regression-runs/{eval_run_id}/impact-analysis
POST /api/feedback-optimization-batches/{batch_id}/regression-runs/{eval_run_id}/gate-overrides
GET  /api/regression-assets
GET  /api/regression-assets/{eval_case_id}
PATCH /api/regression-assets/{eval_case_id}
POST /api/regression-assets/{eval_case_id}/promote
POST /api/regression-assets/{eval_case_id}/archive
POST /api/regression-assets/{eval_case_id}/mark-flaky
POST /api/regression-assets/{eval_case_id}/unmark-flaky
POST /api/regression-assets/{eval_case_id}/supersede
GET  /api/regression-assets/{eval_case_id}/revisions
GET  /api/regression-assets/{eval_case_id}/governance-events
```

### 18.11 Agent Governance
...
GET  /api/agent-change-sets/{change_set_id}/file-diff
...
7. 验证方式 validation
8. 风险 risk
9. 任务动作：编辑、执行、跳过不可执行项、转外部治理或一键执行
```
...
```text
${HOST_RUNTIME_VOLUME_ROOT}/main-workspace/
${HOST_RUNTIME_VOLUME_ROOT}/attribution-analyzer-workspace/
${HOST_RUNTIME_VOLUME_ROOT}/proposal-generator-workspace/
${HOST_RUNTIME_VOLUME_ROOT}/execution-optimizer-workspace/
${HOST_RUNTIME_VOLUME_ROOT}/claude-roots/main/
${HOST_RUNTIME_VOLUME_ROOT}/claude-roots/attribution-analyzer/
${HOST_RUNTIME_VOLUME_ROOT}/claude-roots/proposal-generator/
${HOST_RUNTIME_VOLUME_ROOT}/claude-roots/execution-optimizer/
${HOST_RUNTIME_VOLUME_ROOT}/data/runtime.sqlite3
${HOST_RUNTIME_VOLUME_ROOT}/data/agent-governance/worktrees/
${HOST_RUNTIME_VOLUME_ROOT}/data/agent-governance/releases/
```
...
2. job 生成通过 `AttributionOutput` 校验的归因结果。
...
```text
用户能回答：哪些反馈 -> 哪些证据 -> 哪些归因 -> 哪些建议 -> 哪些任务被确认执行 -> 修改了哪个版本。
```
...
13. 未经任务执行确认的 proposal 修改主智能体文件。
```
...
[ ] 旧版数据目录不会被新版闭环读取。
[ ] 旧版测试已删除或改写为新版链路测试。
[ ] HOST_RUNTIME_VOLUME_ROOT/main-workspace 已替代旧 workspace。
[ ] governor-workspace 已创建（Governor 合并后单一治理 Agent workspace；合并前的五个 *-workspace 已并入，见 §0）。
[ ] claude-roots/main、claude-roots/governor 已创建。
[ ] main-agent 与 governor 两个 profile 的 cwd 不同。
[ ] main-agent 与 governor 两个 profile 的 HOME 不同。
[ ] main-agent 与 governor 两个 profile 的 CLAUDE_CONFIG_DIR 不同。
...
> Governor 合并（Issue #3）后，下述“六套 Runtime Profile”及按归因 / 方案 / 执行 / 用例治理 / 回归影响分析分述的 workspace 命名为合并前设计；运行态已合并为 `main-agent` + `governor` 两套 profile（单一 `governor-workspace` / `claude-roots/governor`），后端按 job_type 复用 `governor`，闭环链路、隔离语义与决议精神不变（详见 §0 与 `docs/engineering/Governor合并设计.md`）。
...
/data/runtime.sqlite3 + main-workspace Git + /data/agent-governance
  = 反馈闭环的权威数据、版本记录和候选/发布资产。
```
