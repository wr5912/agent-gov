...
> 术语口径：本文保留 `main-agent`、`feedback signal`、`feedback case`、`optimization batch`、`proposal-generator` 等当前架构和代码事实；v2.7 用户主流程术语以 [AgentGov术语与版本边界](./AgentGov术语与版本边界.md) 为准。
> 归档边界：本文仍解释当前闭环架构、运行隔离和迁移风险，暂不移动；若未来被 v2.7 架构文档完全替代，应先迁移引用和测试锚点。
> v2.7 覆盖规则：本文保留当前闭环架构和运行隔离事实；若本文按旧批次、旧治理 Agent 职责名、旧版本管理或旧回归资产入口描述用户主流程，全部让位于 [AgentGov v2.7 四阶段改进治理工作台 UI 整改方案](./AgentGov_v2.7_四阶段改进治理工作台UI整改方案.md) 及四张效果图。
