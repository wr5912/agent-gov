...
| **[D-H1]** README 接口清单缺项 | 无"新增路由必同步 README/自动校验"硬约束 |
...
- 涉及外部输入（路径、URL、JSON schema）：**至少 1 个无效/异常输入测试**
- happy path 测试不计入"测试覆盖"，仅作为基线

### B. 可机器执行的护栏（最关键，光靠指令记不住）

在 `.codex/hooks.json` 和仓库内验证脚本中增加以下"硬门"：
...
```bash
# 每次 backend schemas 变更，由仓库内验证脚本重新生成 frontend/src/types/api.ts
...
| 2. 软门接入 | 3 天 | B.1 / B.2 / B.3 接到本地验证入口，但**只警告不阻塞**；建 `.codex/size-budget.yaml` 写入当前所有超限文件作为基线 | 本地验证跑通 |
| 3. 文档同源 | 3 天 | D 节的 README 路由自动生成 + OpenAPI → TS 类型生成；DEFAULT_ALLOWED_TOOLS、反馈优化产品调整方案 等已知漂移 fix | `git diff` 干净 |
| 4. 硬门切换 | 7 天后 | B.1 / B.2 / B.3 从警告升为阻塞；新文件不享豁免 | 本地门禁红绿可验证 |
...
**为什么先软门再硬门**：现存仓库已有大量超阈值文件，立刻硬门会让所有改动无法通过。软门期把当前状态固化为 baseline，新文件零容忍，旧文件按预算偿还。
...
2. **AI 不会自己跨任务回顾**。除非外部机制（本地门禁、size budget、hooks）逼它停下来看全局，否则它每次都只看眼前。这是 AI 协作范式的内禀属性，不是这套指令的特有缺陷。
