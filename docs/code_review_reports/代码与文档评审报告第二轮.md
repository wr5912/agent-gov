...
| 文档一致性 | 🟨 一般 | 🟨 一般 | `开放接口规范.json`/README 与代码已同步；但治理与产品文档**自身**已 stale(CD-1~6) |
...
> 范围：`README.md, docs/*.md, docs/开放接口规范.json ↔ 代码`

**维度小结**：双向核对后整体一致性良好：docs/开放接口规范.json 与真实路由完全同步（70 条路径逐一比对，仅 include_in_schema=False 的 GET / 被正确排除），README 的路由索引/env 默认值/Agent profile/四套 workspace 命名均与 settings.py、.env.example、docker-compose.yml 一致，架构文档 §18 API 设计与代码逐条吻合；问题集中在三类「过期/未随重构校准」的文档：反馈优化产品调整方案 的 API 清单有错名+不存在的路由且状态机命名与权威 state_machines.py 完全不符、代码与文档评审报告 的总览/结论仍以已不存在的「5022 行上帝模块」为现状、架构文档 §5 目录树 claude-roots 命名与代码及其自身 §6 矛盾、智能体治理 对 .codex 现状的描述已被后续提交推翻。
...
- **原则**：一致性
- **证据**：文档 §6.3 列出 `POST /api/feedback-optimization-batches/{batch_id}/optimization-plans`（复数 plans）与 `GET /api/feedback-optimization-batches/{batch_id}/execution-jobs`。代码 app/routers/feedback_batches.py:101-102 实际路由为单数 `/feedback-optimization-batches/{batch_id}/optimization-plan`；对 batch 的 `GET .../execution-jobs` 在 app/ 与 docs/开放接口规范.json 中均不存在（grep 命中 0）。
- **修复**：把 §6.3 第 426 行 `optimization-plans` 改为单数 `optimization-plan`，并删除第 429 行不存在的 `GET .../{batch_id}/execution-jobs`（execution-jobs 实际挂在 optimization-tasks 下）。该文档头部声明「v1 已实现并持续校准」，API 段应以 docs/开放接口规范.json 为准逐条核对。
...
- **原则**：一致性
- **证据**：§18.11 列出 6 条 agent-versions 路由含 `GET /api/agent-versions/main/diff`，但未列 `GET /api/agent-versions/main/file-diff`。代码 app/routers/agent_versions.py 与 docs/开放接口规范.json 均有 file-diff（GET /api/agent-versions/main/file-diff），README:145 也已列出。
- **修复**：在 §18.11 补上 `GET /api/agent-versions/main/file-diff`，与 README 和 开放接口规范.json 对齐；或在 §18 顶部声明完整接口以 开放接口规范.json 为准（README 已采用此声明）。
