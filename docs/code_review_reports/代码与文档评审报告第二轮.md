...
> 与第一轮关系：本报告取代 [`代码与文档评审报告.md`](../archive/review/代码与文档评审报告.md)（R1，重构前基线）。配套治理反思见 [`智能体治理反思与改进方案第二轮.md`](../codex_setting_review_reports/智能体治理反思与改进方案第二轮.md)
...
| 文档一致性 | 🟨 一般 | 🟨 一般 | 运行时 OpenAPI/README 与代码已同步；但治理与产品文档**自身**已 stale(CD-1~6) |
...
- **原则**：一致性
- **证据**：def require_api_key(...): ... raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid API key")。而 register_error_handlers 对 FeedbackStoreError 统一返回 {"detail": ..., "error_code": ...}（error_handlers.py:12-17），docs/archive/review/代码与文档评审报告.md:379 明确记录了"统一 exception handler 返回 error_code 与 detail"的治理目标。
...
- **原则**：鲁棒性/一致性（幂等）
- **证据**：先 `existing = self.find_event(req.event_id)`（160 行，独立读），未命中后在另一个事务块里 `db.add(SocEventModel(event_id=event["event_id"], ...))`（182 行）。同文件 create_signal 用的是 `db.merge(...)`（113 行）。docs/archive/obsolete/反馈优化闭环多智能体架构.md:1301 'event_id 必须由 source_system 保证幂等'、1653 'POST /api/soc-events 对 event_id 幂等'。
...
- **修复**：为 ready->applied 引入应用级幂等键或 SELECT...FOR UPDATE 等价的 compare-and-set（在落库事务内重新校验 status==ready 且 applied_agent_version_id 为空，否则中止），或对单个 task 加 asyncio.Lock/进程内互斥；把'写文件成功后必须落库'用 outbox/saga 表达，而非仅靠事后补偿。docs/archive/review/代码与文档评审报告.md B-S4 已记录此问题但代码未修复。

#### [FO-2] 🟨 中 编排器四个 run_*_job 方法大段样板重复（DRY），新增 Job 类型需复制整套 try/except 流程

- **位置**：`app/runtime/feedback_job_orchestrator.py:41-190`
- **原则**：DRY/扩展性（开闭原则）
- **证据**：run_attribution_job / run_proposal_job / run_batch_optimization_plan / run_execution_job 四个方法结构几乎逐行相同：create_*_job -> `if job.get("_reused_existing") or job.get("status") != "queued": return ...` -> start_*_job -> `try: raw = await self.run_profile_json(...); complete_*(raw) except asyncio.TimeoutError: fail_*(... AGENT_TIMEOUT) except Exception: fail_*(... AGENT_RUNTIME_ERROR)`。仅 profile_name / prompt / OutputModel / job_type / ID 键不同。
- **修复**：抽出一个泛化 _run_job(job, *, profile_name, build_prompt, schema_version, job_type, start_fn, complete_fn, fail_fn, result_fn) 模板方法，或为每种 job 定义一个声明式 JobSpec/JobExecutor 多态对象（与文档 B-S2 拆分计划一致），让新增 Agent/Job 只需注册一个 spec 而非复制 ~40 行。
...
- **位置**：`docs/archive/obsolete/反馈优化产品调整方案.md:524-560`
- **原则**：一致性（实现与 docs 矛盾）
...
**维度小结**：外部治理服务的对外门面（mixin）、Langfuse 适配器与 message_utils 拆分边界总体清晰，禁用时 Langfuse 与 DSPy 格式化器均优雅降级（返回 None/nullcontext，build_env 仅在显式启用且缺密钥时 fail-fast），webhook 配置加载具备 YAML/类型/必填校验；但 ExternalGovernanceItemModel 的 row↔record 映射在拆分后被三个 store 模块各自重复实现（机械碎片化迹象），通知路径缺少对 superseded 终态的状态机保护、且 notify-then-persist 顺序在崩溃时会破坏“通知必落库”不变量，notification_payload 与 record 字段存在双处维护风险。
...
- **位置**：`app/runtime/external_governance.py, docs/archive/obsolete/反馈优化闭环多智能体架构.md:external_governance.py:101-140; doc:1579`
...
> 范围：`README.md, docs/*.md, 运行时 `/openapi.json` 或临时导出 OpenAPI JSON ↔ 代码`

**维度小结**：双向核对后整体一致性良好：运行时 `/openapi.json` 或临时导出 OpenAPI JSON 与真实路由完全同步（70 条路径逐一比对，仅 include_in_schema=False 的 GET / 被正确排除），README 的路由索引/env 默认值/Agent profile/四套 workspace 命名均与 settings.py、.env.example、docker-compose.yml 一致，架构文档 §18 API 设计与代码逐条吻合；问题集中在三类「过期/未随重构校准」的文档：反馈优化产品调整方案 的 API 清单有错名+不存在的路由且状态机命名与权威 state_machines.py 完全不符、代码与文档评审报告 的总览/结论仍以已不存在的「5022 行上帝模块」为现状、架构文档 §5 目录树 claude-roots 命名与代码及其自身 §6 矛盾、智能体治理 对 .codex 现状的描述已被后续提交推翻。

#### [CD-2] 🟨 中 反馈优化产品调整方案 §8 状态机命名与权威 state_machines.py 完全不符

- **位置**：`docs/archive/obsolete/反馈优化产品调整方案.md:528-556`
- **原则**：一致性
...
#### [CD-3] 🟨 中 代码与文档评审报告 总览表与「重点结论」仍以已不存在的 4 个上帝模块为现状

- **位置**：`docs/archive/review/代码与文档评审报告.md:14,31,321`
- **原则**：一致性
...
#### [CD-1] 🟩 低 反馈优化产品调整方案 §6.3 API 清单含错名路由与不存在的路由

- **位置**：`docs/archive/obsolete/反馈优化产品调整方案.md:426,429`
- **原则**：一致性
- **证据**：文档 §6.3 列出 `POST /api/feedback-optimization-batches/{batch_id}/optimization-plans`（复数 plans）与 `GET /api/feedback-optimization-batches/{batch_id}/execution-jobs`。代码 app/routers/feedback_batches.py:101-102 实际路由为单数 `/feedback-optimization-batches/{batch_id}/optimization-plan`；对 batch 的 `GET .../execution-jobs` 在 app/ 与 运行时 `/openapi.json` 或临时导出 OpenAPI JSON 中均不存在（grep 命中 0）。
- **修复**：把 §6.3 第 426 行 `optimization-plans` 改为单数 `optimization-plan`，并删除第 429 行不存在的 `GET .../{batch_id}/execution-jobs`（execution-jobs 实际挂在 optimization-tasks 下）。该文档头部声明「v1 已实现并持续校准」，API 段应以 运行时 `/openapi.json` 或临时导出 OpenAPI JSON 为准逐条核对。

#### [CD-4] 🟩 低 架构文档 §5 目录树 claude-roots 命名与代码及其自身 §6 矛盾

- **位置**：`docs/archive/obsolete/反馈优化闭环多智能体架构.md:318-324`
- **原则**：一致性
...
#### [CD-5] 🟩 低 智能体治理 §5.C 称 .codex/rules 与 hooks 为空，已被后续提交推翻

- **位置**：`docs/archive/retrospective/智能体治理反思与改进方案.md:227,230`
- **原则**：一致性
...
- **位置**：`docs/archive/obsolete/反馈优化闭环多智能体架构.md:1471-1478`
- **原则**：一致性
- **证据**：§18.11 列出 6 条 agent-versions 路由含 `GET /api/agent-versions/main/diff`，但未列 `GET /api/agent-versions/main/file-diff`。代码 app/routers/agent_versions.py 与 运行时 `/openapi.json` 或临时导出 OpenAPI JSON 均有 file-diff（GET /api/agent-versions/main/file-diff），README:145 也已列出。
- **修复**：在 §18.11 补上 `GET /api/agent-versions/main/file-diff`，与 README 和 运行时 OpenAPI 对齐；或在 §18 顶部声明完整接口以 运行时 OpenAPI 为准（README 已采用此声明）。
...
3. **[已完成] 执行应用幂等与原子性**：`ExecutionApplicationService` 已加应用锁、ready 前置校验、修复前版本校验、应用后快照、失败补偿记录与日志留痕；`mark_execution_job_applied` 已在 DB 事务内对 execution job / task 重新加锁校验，并通过 `status='ready'` 条件更新阻止重复应用覆盖第一次结果；批次路由已委派 `run_and_apply_execution_job`。（FO-1 / FO-5 / BA-1）
4. **[已完成] 收紧过期体积基线与自动门**：当前仓库不再使用 `.codex/size-budget.yaml`；治理硬门统一由 `scripts/check_codex_governance.py --mode fail` 对比 git base 检查行数、函数、类、路由和状态机缺表；Stop hook 与 `make test` 已接入同一硬门。
...
7. **[已完成] 编排器去样板**：`FeedbackJobOrchestrator` 已抽 `_run_profile_json_job`，四类 Agent job 共享异常映射、schema 期望与完成/失败收口。（FO-2）
...
14. **[已完成] 文档校准**：`反馈优化产品调整方案`、架构文档目录树、README 与 OpenAPI/types 已同步到当前路由、状态机和目录结构。（CD-1~6）
...
4. **根因不在某段代码，而在治理闭环**：详见配套的 [`智能体治理反思与改进方案第二轮.md`](../codex_setting_review_reports/智能体治理反思与改进方案第二轮.md)——护栏需从"数行数"升级为"查结构"，并从"人工 warn"升级为"提交和发版前强制"。
