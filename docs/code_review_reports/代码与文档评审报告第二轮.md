...
- **原则**：鲁棒性/一致性（幂等）
- **证据**：先 `existing = self.find_event(req.event_id)`（160 行，独立读），未命中后在另一个事务块里 `db.add(SocEventModel(event_id=event["event_id"], ...))`（182 行）。同文件 create_signal 用的是 `db.merge(...)`（113 行）。docs/反馈优化闭环多智能体架构.md:1301 'event_id 必须由 source_system 保证幂等'、1653 'POST /api/soc-events 对 event_id 幂等'。
...
- **位置**：`app/runtime/external_governance.py, docs/反馈优化闭环多智能体架构.md:external_governance.py:101-140; doc:1579`
...
- **位置**：`docs/反馈优化闭环多智能体架构.md:318-324`
- **原则**：一致性
...
- **位置**：`docs/反馈优化闭环多智能体架构.md:1471-1478`
- **原则**：一致性
