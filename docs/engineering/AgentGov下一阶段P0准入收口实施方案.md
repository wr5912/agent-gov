...
P0 解决“平台主门已绿，但旗舰安全 Agent 自测仍红且质量策略不可见”的准入矛盾，并冻结后续
Runtime 与评测实现必须共同遵守的生命周期语义。

退出时必须同时得到：

- `security-operations-expert` 当前 Workspace 静态测试 29/29 通过；
- 该测试集被 `tests/quality_policy.json` 收集和分类，不能再绕过主质量清单；
...
- 根质量策略 collection 只覆盖根 `tests` 与 Governor Workspace 测试，未收集该安全 Agent 测试；
...
- 多 Runtime 目标方案允许历史 run 恢复为 `running`，而当前集中状态机坚持一次 run 不可重开。
...
| 测试组合 | `tests/quality_policy.json` | 增加 owner、capability、lane 和 resource 分类 |
...
- 将安全 Workspace 静态测试加入根 collection；
- 新建或扩展 portfolio rule：
  - owner：`security-response`；
  - capabilities：`security-boundaries`、`agent-lifecycle`；
  - lane：`main-full`；
...
1. 一个部署只选择一个受管 Runtime；客户端请求不能覆盖。
...
该裁决替换多 Runtime 目标方案中 `interrupted -> running` 的表述；获批后应同步回写目标方案，
避免两个活跃真相源。
...
| --- | --- | --- | --- |
| P1 安全测评 | AGV-002、009、028、035、043、046 | `improvement-governance` + `security-response` | Workspace 测评、改进闭环、精确发布 |
| P2A Runtime | AGV-002、014、040、050 | `runtime-platform` + `integrations` | contract、SSE/resume/HITL、真实容器 |
| P2B Governor | AGV-006、009、010、012、019、045 | `improvement-governance` | 不可变证据、隔离评估、shadow 不生效 |
...
| 根 collection 纳入安全测试 | `PROMOTE` | quality policy manifest 测试 | 收集和分类都必须成立 |
...
3. `.venv/bin/python scripts/check_test_quality_policy.py --manifest-only`；
...
- 安全 Workspace 测试仍不在根 collection；
