...
| 当前实现边界 | 49 `current`，0 `gap`、0 `future`：全部 49 个 AGV 用例达 `current`。AGV-049（外部协作平台对接晚于核心治理稳定）的成功标准是"边界纪律"——外部协作深度对接被显式 deferred 即为达成，已由文档阶段声明 + 自动验收背书 |
...
| 状态 | 数量 | 含义 | 在本计划中的角色 |
| --- | --- | --- | --- |
| `current` | 49 | 当前应具备 | 回归锚点，任何阶段不得退化 |
| `gap` | 0 | 目标明确、能力不足 | 阶段1已清零 |
| `future` | 0 | 长期愿景/成熟度 | 全部用例已达 `current` |
...
截至 2026-06-15，49 个 AGV 用例全部为 `current`，愿景在可验收意义上达成。闭环对象（多业务 Agent）、三层资产（数据/方法论/执行）、反馈→归因→优化→评估→版本全链路 per-agent 治理、场景包与跨 Agent 方法论复用均已 `current` 并经 `make test` 离线硬门守护。

关键认知（AGV-049）：最后一个用例"外部协作平台对接晚于核心治理稳定"的成功标准是**边界纪律**而非交付外部集成——其要求外部协作平台深度对接被显式归入长期生态阶段、不提前拉起；**实现该集成反而违反该用例并破坏离线不变量**。因此 49/49 是诚实达成：48 个能力用例由实现 + 自动验收背书，AGV-049 由文档阶段声明 + 边界自动验收背书。后续保持回归不退化；外部协作深度对接仍待前三个产品大版本稳定后再设计，且不得破坏"离线模式 + 必需工作流不依赖远程服务"不变量。
...
| 2026-06-12 | AGV-026 | 关闭（stage-2 第4个）：场景包/能力域子系统——ScenarioPack 实体(业务目标/范围/风险)+迁移0010；POST/GET /api/scenario-packs、/{id}/assets(Agent 装配)、/{id}/copy(资产复制/迁移为模板)；关联可查=可审计 | 通过 | `future` → `current` | 用例文档 AGV-026 `自动验收`、`tests/test_scenario_pack_store.py` |
| 2026-06-12 | AGV-023 | 关闭（stage-2 第5个）：场景包资产去重——`GET /api/scenario-packs/duplicates` 按规范化名检测+建议主资产；`POST /{primary}/merge` 并入主资产、重复包标记 merged_into 保留可审计、关联去重并集引用不丢失 | 通过 | `future` → `current` | 用例文档 AGV-023 `自动验收`、`tests/test_scenario_pack_store.py` |
...
| 2026-06-15 | AGV-049 | 关闭（愿景收官 49/49）：外部协作平台对接晚于核心治理稳定——其成功标准是边界纪律（外部协作深度对接显式 deferred、不提前拉起），由 vision/README/执行计划阶段声明 + 自动验收背书；实现该集成反而违反用例并破坏离线不变量 | 通过 | `future` → `current` | 用例文档 AGV-049 `自动验收`、`tests/test_agv_acceptance.py::test_agv_049_external_collaboration_integration_is_deferred_long_term_stage` |
...
| 2026-06-12 | AGV-017 | 关闭（B3.4 首个用例）：多业务 Agent 接入统一治理闭环——两个业务 Agent 各 run/反馈/优化/评估/版本独立闭环互不混淆、按 Agent 维度过滤、单一治理服务多 Agent、版本链物理隔离 | 通过 | `future` → `current` | 用例文档 AGV-017 `自动验收`、`test_governance_serves_multiple_business_agents_with_isolated_closed_loops` |
| 2026-06-12 | AGV-021 | 关闭（B3 首个用例）：业务 Agent 生命周期围绕版本治理运转——候选/已发布/回滚版本状态可区分；restore 切换当前版本不改写 release 历史；rollback 仅标记 `rolled_back` 不物理删除 release；两条 release 在 Agent 维度仍可追溯，per-agent 版本链与 main 物理隔离 | 通过 | `future` → `current` | 用例文档 AGV-021 `自动验收`、`test_business_agent_version_lifecycle_preserves_history_through_rollback` |
