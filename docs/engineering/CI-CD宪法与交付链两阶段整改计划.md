...
> 文档状态：当前工程宪法。
>
> 本文只定义产品工程、业务 Agent workspace、CI/CD 的稳定原则、权威顺序和修订边界。
...
- `master` 必须经 PR、唯一 AID 和 required `quality-gate`。
- 绿色 CI 不自动部署。
- 联调部署只允许显式调用 `scripts/deploy_agent_gov_to_host`。
- 部署必须绑定完整 SHA、AID、PR 和成功 workflow URL。
...
4. workflows、Makefile 和 scripts；
5. GitHub、Multica、目标主机等实际运行证据。
