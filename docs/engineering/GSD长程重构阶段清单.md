...
- 涉及 `${HOME}/volume-agent-gov` 默认数据、`docker/volume` 历史数据、SQLite payload、agent job 快照或旧 API 契约。
...
- [ ] 写明真实数据样本来源，尤其是 `${HOME}/volume-agent-gov/data`、迁移前 `docker/volume/data` 与历史 SQLite。
- [ ] 写明本阶段要新增或收紧哪些治理硬门。
...
| 浏览器 smoke | `make ui-feedback-smoke`（`make ui-design-parity` 是其真实容器别名）；禁止伪 API、请求拦截或预制页面状态 |
