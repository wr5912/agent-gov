...
| 部署脚本 | 精确 SHA 联调部署、健康检查、诊断和回退 | 监听 GitHub、无人值守决策或部署版本 |
...
绿色 CI 不触发部署。无参数调用会解析并打印当前 `origin/master` tip，再按该 SHA
反查成功的 `master` push `quality-gate` run：

```bash
scripts/deploy_agent_gov_to_host
```

需要钉住某个可从 `origin/master` 到达的完整 SHA 时使用 `--ref`；`--workflow-url`
可作为显式定位符。若该 SHA 是 PR merge commit，可成对提供 `--aid` 和
`--pr-number` 追加 PR/AID 绑定：
...
省略 `--workflow-url` 时必须真正不向校验器传递该参数，由校验器按 SHA 反查；
不得把空字符串当成显式 URL。`--aid` 与 `--pr-number` 只能成对提供。
`--preflight-only`、`--remote-status`、诊断和回退等动作按脚本自身参数契约执行，
不伪造一次新部署。
...
- 提供 PR/AID 时，该 SHA 恰好是所填 PR 合入 `master` 的 merge commit，且
  PR head branch 中的唯一 AID 与命令参数一致。
...
首次接管尚无 `current` 的旧 Compose 部署时，helper 会尝试建立 `legacy-bootstrap`
回滚快照。旧镜像已不可用时只输出 `WARN` 并继续部署，不把“无 legacy 回滚目标”
当成发布前置阻断。新版本自身的镜像加载、Compose 启动和 API/UI/Langfuse 健康检查
仍然失败关闭；若当时无可用回滚快照，失败必须明确返回，不伪报已恢复。
回滚快照只覆盖发布代码、Compose 与镜像；`shared/docker.env` 和
`${HOME}/volume-agent-gov` 始终是共享运行边界，不随发布快照自动回滚。
持久化 Postgres、Redis 或 MinIO 的凭据轮换必须同时修改存储内凭据与所选 env，
不能只改 env 文件后重建容器。
...
- 无参数调用能解析 `origin/master` tip 并按 SHA 反查成功 run；显式错误 SHA、
  workflow URL 或只提供 AID/PR 一侧时在接触目标机前失败；
- 失败/错误 attempt、非 `master` push、错误 workflow、错误 SHA、未合并或不匹配的
  PR/AID 都会在接触目标机前失败；
- 精确 SHA 的预检、部署、健康检查和远端状态查询通过；
- 有可用快照时，失败部署能诊断并恢复先前健康快照；无 legacy 快照时
  发出告警后继续部署，但不伪造回滚保证；
