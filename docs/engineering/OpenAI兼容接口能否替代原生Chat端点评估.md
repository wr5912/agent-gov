...
- `alert_id`、`case_id`、`source`、`client_run_label` 属于业务/观测元数据，优先放入 OpenAI 标准 `metadata`，不再放入 `agentgov`。
...
| `instructions` | OpenAI 标准 | 追加型系统/开发者指令；映射到现有 `ChatRequest.system_append` 语义，不覆盖 workspace `CLAUDE.md` 或 AgentGov preset |
...
| `metadata` | OpenAI 标准 | 扁平业务/观测元数据；承载 `alert_id`、`case_id`、`source`、`client_run_label` 等 |
...
> `input` 为 items 数组时，服务端取其文本/内容映射为本轮 Claude Code prompt。`instructions` 必须使用 OpenAI 标准字段，但 AgentGov 的 system 身份仍以 Claude Code preset + workspace `CLAUDE.md` 为治理态单一真相源；因此 `instructions` 在本项目中只作为 append-only 追加指令，不具备替换受治理 prompt 的能力。不得再新增 `agentgov.system_append` 这类平行字段。

模式口径：

- **strict mode**：请求不含 `agentgov`。该模式是 OpenAI Responses-compatible 子集，使用运营者配置的 OpenAI-compat Agent，不发 `agentgov.*` 私有事件。
- **control mode**：请求含 `agentgov`。该模式启用 AgentGov 控制面，`agentgov.agent_id` 必填，可使用 HITL、工具时间线、raw SDK 调试和反馈治理投影。
- 标准字段在两种模式下含义一致；`agentgov` 不得复制、改名或覆盖已有 OpenAI 标准字段。
...
- 内部可映射到 `/v1/responses` strict mode adapter，但不暴露 `agentgov.*` 控制事件。
...
3. **新增 `/v1/responses/{id}` retrieve**：从 `resp_<run_id>` 解析现有 run/session 投影；实现 `store` 与 `previous_response_id` 的最小兼容语义。
4. **新增 `/v1/conversations`**：把会话列表、会话删除、历史读取从 `/api/sessions*` 迁移为 Conversations/Items 契约。
5. **前端主路径切换**：Playground 调 `/v1/responses stream=true`，会话侧栏与历史恢复调 `/v1/conversations*`。
6. **兼容层回归**：`/v1/chat/completions`、`/api/chat`、`/api/chat/stream` 全部走新 adapter 或等价逻辑，保证旧调用方不破。
7. **弃用评估另行立项**：只有新主路径真实容器验收通过，并确认外部消费者迁移后，才评估是否给旧端点加 deprecation/sunset 策略。
...
- 标准字段优先级：`instructions`、`metadata.alert_id/case_id`、`conversation`、`previous_response_id` 不被 `agentgov` 平行字段替代；请求包含 `agentgov.instructions`、`agentgov.metadata`、`agentgov.conversation` 等字段时应被 schema 拒绝。
