...
- **strict mode**：请求不含 `agentgov`。该模式**标准字段形状兼容** OpenAI Responses，使用运营者配置的 OpenAI-compat Agent，不发 `agentgov.*` 私有事件。**但不是纯 OpenAI 语义子集**：AgentGov 的 system 身份以 Claude Code preset + workspace `CLAUDE.md` 为单一真相源，`instructions` 为 **append-only**、不实现官方 replace/swap 语义——strict 模式对 `instructions` 应**拒绝（422）或忽略并回显告警**，不静默按追加生效，避免纯 OpenAI 客户端误以为能替换/清空系统身份。
...
6. **兼容层回归**：`/v1/chat/completions`、`/api/chat`、`/api/chat/stream` 全部走新 adapter 或等价逻辑，保证旧调用方不破。`/api/chat`（control、`agent_id` 必填 422）与 `/v1/chat/completions`（strict、无 `agent_id`）走同一 adapter 时，mode 与必填语义差异须在契约测试中**各自断言**，不靠单一 smoke；旧事件名与 payload 字段集须回归覆盖。
