...
`response.id` 可以由 `run_id` 稳定派生，避免新增并行存储；真正的会话历史仍以 Claude SDK / agent transcript 为单一真相源。
...
- 优先复用 claude-agent-sdk session API 与当前 `read_session_history` 投影能力。
...
- 保留 `decision_token`，作为 API key 之外的 per-request 授权因子。
- `request_id` 定位 record，`decision_token` 做 constant-time 校验。
- `extra=forbid`，拒绝 `updated_input`、`allow_modified` 等未设计字段。
- `allow_once`、`allow_for_run`、`deny`、`answer_question` 语义保留。
