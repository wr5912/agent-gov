...
| Claude Code / Agent job | container 或 local-debug | `MODEL_PROVIDER_API_URL` 指向真实模型服务；`ANTHROPIC_BASE_URL` 由 Runtime 派生 | profile workspace / data_dir | warning/API/health 诊断脱敏；自托管 Langfuse 开发 trace 保留完整 prompt/tool/job I/O | Agent job precheck、main-flow test |
...
| 目标模型无法完成类 Claude Code 工具循环 | 阻止 Agent job / Claude Code 执行路径 | 返回 `MODEL_AGENT_LOOP_CAPABILITY_FAILED` |
| 目标模型无法输出 schema-exact JSON | 阻止 governor job / formatter 依赖路径 | 返回 `MODEL_SCHEMA_EXACT_OUTPUT_FAILED` |
...
- warning/API/health 诊断：只记录脱敏后的 route、probe、code、duration，不写 raw prompt 或 tool 参数；自托管 Langfuse 开发 trace 遵循项目调试面原则，保留完整 prompt、tool input/output、job input/output、raw text 和 trace I/O。
...
| 阶段 | 目标 | 主要变更 | 验收 |
| --- | --- | --- | --- |
| P0 文档与配置边界 | 固化禁止手配版本、单 URL、阈值可配和 warning 规则 | 更新 docs、env 示例和配置说明；禁止 `MODEL_PROVIDER_VLLM_VERSION` | 文档检查通过，env 示例无手配版本 |
| P1 真实请求抓包与回放 | 用真实 Claude Code 请求决定接入路径 | 脱敏抓包、请求回放、system 语义保持检查、失败点清单 | 不兼容点完整记录；不能只记录第一个 400 |
| P2 Provider probe | 新增 vLLM `/version`、`/v1/models`、chat/tool/streaming capability probe | probe service、脱敏 warning、TTL 限频、结构化 probe result、`MODEL_PROVIDER_BACKEND` adapter 选择 | 单测覆盖版本成功、失败、不可解析、低版本 |
| P3 LiteLLM sidecar | 接入 LiteLLM 作为唯一协议转换 sidecar | LiteLLM service、healthcheck、由 `MODEL_PROVIDER_API_URL` 派生模型服务后端地址、SSE error 和错误映射 | fake vLLM 集成测试通过；Claude Code tool/streaming/SSE error 不通过时报 `LITELLM_CLAUDE_CODE_COMPAT_FAILED` |
| P4 模型能力预检 | 前置验证目标模型是否可支撑 Claude Code / AgentGov 工作负载 | 类 Claude Code 大 system、多工具循环、schema-exact JSON、乱循环检测 | C 类错误与协议错误分离 |
| P5 Runtime 路由与 Formatter 收敛 | Runtime/Agent job/Claude Code/formatter 统一使用 provider route | 派生 `ANTHROPIC_BASE_URL`；低版本和未知版本强制 sidecar；移除 URL 字符串 provider 推断 | main-flow test、formatter 单测和批次 job 测试通过，job error_json 可见 |
| P6 模型服务连通性与能力验收用例 | 建立真实本地 vLLM gated live 验收 | S1/S2/C 三层验收和端到端容器 live 确认 | 无凭据 skip；配置后 live pass 或给明确错误 |
| P7 收尾同步 | README、docs、skill 和发布说明同步 | README 模型接入运行说明、docs 索引、文档/Codex 硬门 | `check_docs_governance` 与 `check_codex_governance` 通过 |
...
- `/version` 超时、404、非 JSON、缺少 `version`：生成 `VLLM_VERSION_PROBE_FAILED`，打印 warning，默认 LiteLLM sidecar。
...
C 类预检要在完整 sidecar 改造前尽早运行。最小用例应直接用一条类 Claude Code 请求打目标模型：包含 claude_code 量级 system、一组工具、一个必须调用工具才能回答的问题，以及一个要求输出 governor schema-exact JSON 的任务。若 C 类预检失败，不能把失败归类为协议问题，也不能继续宣称该模型可用于 Agent job。
