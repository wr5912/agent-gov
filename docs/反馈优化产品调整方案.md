...
2. worker 调用 `eval-case-governor`，输出由后端规范化并校验为 `feedback-eval-case-generation-output/v1`。
...
2. 读取批次内 eval cases。
3. 接口排队 `proposal-generator` agent job；worker 产出优化方案内容，并由后端规范化为 `feedback-optimization-plan-output/v1`。
...
1. 批次级统筹归因结果，由 `proposal-generator` 产出方案内容并规范化为 `feedback-optimization-plan-output/v1`。
