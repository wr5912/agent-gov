...
4. **B3.4-exec（主体已落地，余 baseline 子片）** 把 main-centric 优化执行流水线参数化为 per-agent。task agent_id 列（迁移 0013）+ 批次 payload agent_id（派生源）+ 执行流水线 store 选择已落地并过 `make test`（490 passed，main 不退化）。**余最后子片**：`OptimizationTask.baseline_agent_version_id` 仍由 main 版本 provider 派生，业务 Agent 首次执行会在 `_prepare_candidate_change_set` 的 baseline 校验处与刚 bootstrap 的业务 store current 版本不一致而报错；需把 task/proposal 的 baseline 也按 agent_id 从对应版本 store 派生（或在创建 task 时按业务 store current 取 baseline），随后业务执行即可端到端落业务 store。下方为已落地两部分的细节：
