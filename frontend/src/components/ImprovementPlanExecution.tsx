...
export function ImprovementPlanExecution({
  item, busy, optPlan, execution, attribution, onGenerateOpt, onConfirmOpt, onRecordExec, onConfirmExec,
}: {
    ...
    <>
      ...
      {execution ? (
        <div className="iw-detail-section" data-testid="execution-record">
          <h4>执行记录{execution.status === "confirmed" ? "（已确认）" : "（待确认）"}</h4>
          ...
          {execution.agent_version ? <div className="iw-list-item-meta">Agent 版本：{execution.agent_version}</div> : null}
        ...
        <div className="iw-detail-section" data-testid="execution-empty">
          <h4>执行记录</h4>
          <div className="iw-next-step">方案已确认。可记录执行结果（应用变更、生成版本）。</div>
          <button className="iw-secondary-button" type="button" data-testid="record-execution" disabled={busy} onClick={onRecordExec} style={{ marginTop: 8 }}>记录执行结果</button>
        </div>
      ) : null}
    </>
  );
