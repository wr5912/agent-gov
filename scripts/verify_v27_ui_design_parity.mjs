...
function defaultPayload(path) {
  ...
  if (path === "/api/agents" || path === "/api/skills" || path === "/api/sessions" || path === "/api/agent-change-sets" || path === "/api/agent-releases") return [];
...
const BASELINE_RULES = new Set(
  (process.env.PARITY_BASELINE || "nav-converged,settings-ia,playground-clean,playground-config-drawer,feedback-drawer-2phase").split(",").map((s) => s.trim()).filter(Boolean),
);
...
const RULES = [
  ...
  { id: "context-4types", phase: "P2", desc: "获取上下文四类型 + 下载", async fn(page) {
    ...
    if ((await first.count()) === 0) return { ok: false, detail: "无改进事项可打开上下文" };
    await first.click();
