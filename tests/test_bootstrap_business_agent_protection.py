...
def test_bootstrap_preserves_optimized_business_agent_and_renders_new_seed(tmp_path: Path) -> None:
    ...
    # 即便最激进的 --overwrite + --repair-managed-config，business-agents 子树也只 fill-missing
    result = bootstrap_runtime_volume(
        runtime_root=root, template_dir=seed, overwrite=True, repair_managed_config=True
    )
    ...
    # AAA 既未出现在 copied 也未出现在 repaired（被保护跳过）
    assert not any("business-agents/AAA/" in p for p in result["copied"])
    assert not any("business-agents/AAA/" in p for p in result["repaired"])
