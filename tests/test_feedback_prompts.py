...
def test_feedback_prompts_spell_out_current_business_information_points():
    ...
    assert "必须且只能调用一次 result = agent.run" in prompts["regression_test_design"]
    assert "禁止 mock、patch、test double" in prompts["regression_test_design"]
    assert "禁止重新赋值 result" in prompts["regression_test_design"]
