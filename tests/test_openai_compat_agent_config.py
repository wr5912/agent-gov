...
重点验证“从未配置（平台默认）”与“显式选择某个业务 Agent”是两个不同状态，
且 /v1 据配置解析 profile（配置失效时 fail-loud 503）。
"""
...
def test_get_unconfigured_uses_platform_default(monkeypatch, tmp_path: Path) -> None:
...
def test_v1_unconfigured_runs_platform_default(monkeypatch, tmp_path: Path) -> None:
    """未配置 /v1 出口时解析并运行平台注册表中的默认业务 Agent。"""
    ...
    with TestClient(module.app) as client:
        ...
        assert resp.status_code == 200
    assert captured["profile"] is not None
    assert captured["profile"].agent_id == DEFAULT_BUSINESS_AGENT_ID
    assert str(captured["profile"].workspace_dir).endswith(f"/business-agents/{DEFAULT_BUSINESS_AGENT_ID}/workspace")
